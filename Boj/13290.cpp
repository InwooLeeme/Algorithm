...
void Main(){ 
	...
	ri(m);
	while(m--){
		int a,b,c; ri(a, b, c); G[a].push_back({c, b}); G[b].push_back({c, a});
	}
	priority_queue<I, vector<I>, decltype(Cmp)> PQ(Cmp); 
	vp dist(n + 1, {INF, 0}); 
	dist[1] = {0, w[1]};
	PQ.push({dist[1].first, w[1], 1});
	while(PQ.size()){
		auto [cdist, pack, cur] = PQ.top(); PQ.pop();
		if(dist[cur].first < cdist) continue;
		for(const auto& [cost, nxt] : G[cur]){
			if(dist[nxt].first > cdist + cost){
				dist[nxt] = {cdist + cost, pack + w[nxt]};
				PQ.push({dist[nxt].first, dist[nxt].second, nxt});
			}
			else if(dist[nxt].first == cdist + cost){
				if(dist[nxt].second < pack + w[nxt]){
					dist[nxt].second = pack + w[nxt];
					PQ.push({dist[nxt].first, dist[nxt].second, nxt});
				}
			}
		}
	}
	if(dist[n].first == INF) po("impossible");
	else po(dist[n].first, dist[n].second);
} 
