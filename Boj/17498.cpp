...
void Main(){
    ...
    vi v(n * m),D(n * m, -INF); int idx = 0;
    for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
        int x; ri(x); v[idx++] = x;
    }
    auto Sol = [&](int cur, auto&& Sol) -> int{
        if(cur >= n*m) return -INF;
        int x = cur / m, y = cur % m;
        if(x == n - 1) return 0;
        int& ret = D[cur];
        if(ret != -INF) return ret;
        for(int r = x + 1; r <= x + d; r++) for(int c = y - d; c <= y + d; c++){
            if(Bound(r, c) or abs(r - x) + abs(c - y) > d) continue;
            auto nidx = r*m + c;
            int temp = Sol(nidx, Sol) + (v[nidx] * v[cur]);
            ret = max<int>(ret, temp);
        }
        return ret;
    };
    int mx = -INF;
