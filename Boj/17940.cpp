...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < n; j++){
		int t; ri(t);
		if(t) G[i].push_back({t, j});
	}
	vi D(1010, INF), v(1010, INF);
	D[0] = 0; v[0] = 0;
	min_pq<ti3> PQ;
	PQ.push({v[0], D[0], 0});
	while(PQ.size()){
		auto [state, cdist, cur] = PQ.top(); PQ.pop();
		...
		for(const auto& [cost, nxt] : G[cur]){
			if(D[nxt] > cost + cdist){
				D[nxt] = cost + cdist;
				auto nstate = state + (w[nxt] != w[cur] ? 1 : 0);
				v[nxt] = nstate;
				PQ.push({v[nxt], D[nxt], nxt});
			}
			else if(D[nxt] == cost + cdist){
				if(v[nxt] > state + (w[nxt] != w[cur] ? 1 : 0)){
					v[nxt] = state + (w[nxt] != w[cur] ? 1 : 0);
					PQ.push({D[nxt], v[nxt], nxt});
				}
			}
		}
	}
