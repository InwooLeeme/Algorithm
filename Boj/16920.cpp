...
void Main(){ 
	ri(n, m, p);
	for(int i = 1; i <= p; i++) ri(S[i]);
	for(int i = 0; i < n; i++) ri(v[i]);
	vector<queue<ti3>> I(p + 1);
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
		if(v[i][j] == '.' or v[i][j] == '#') continue;
		for(int d = 0; d < 4; d++){
			auto nx = i + dx[d], ny = j + dy[d];
			if(Bound(nx, ny)) continue;
			if(v[nx][ny] == '.'){
				I[v[i][j] - '0'].push({0, i, j});
			}
		}
	}
	while(1){
		bool flag = 0;
		vector<queue<ti3>> Q(p + 1);
		for(int k = 1; k <= p; k++){
			while(I[k].size()){
				auto [cdist, x, y] = I[k].front(); I[k].pop();
				if(cdist >= S[k]) {Q[k].push({0, x, y}); continue;}
				for(int d = 0; d < 4; d++){
					auto nx = x + dx[d], ny = y + dy[d];
					if(Bound(nx, ny) or v[nx][ny] == '#') continue;
					if('1' <= v[nx][ny] and v[nx][ny] <= '9') continue;
					v[nx][ny] = k | 48;
					flag = 1;
					I[k].push({cdist + 1, nx, ny});
				}
			}
			while(Q[k].size()){
				auto [cdist, x,y] = Q[k].front(); Q[k].pop();
				I[k].push({cdist, x, y});
			}
		}
		if(!flag) break;
