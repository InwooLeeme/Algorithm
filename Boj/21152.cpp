...
void Main(){
	int N; ri(N);
	map<string, string> G,M;
	set<string> S;
	for(int i = 0; i < N; i++){
		string _,a,b; ri(_,a,b);
		M[a] = b;
		S.insert(a);
	}
	...
	for(const auto& [a, b] : G){
		if(vist.count(a)) continue;
		int cnt = 0;
		set<string> vist1;
		auto cur = a;
		while(!vist1.count(cur)){
			vist1.insert(cur);
			if(G.count(cur)){
				cur = G[cur];
				cnt++;
		...
		if(cur == a) mx = max<int>(mx, cnt);
		for(const auto& j : vist1) vist.insert(j);
	}
	if(mx == -1) po("No trades possible");
	else po(mx);
}
