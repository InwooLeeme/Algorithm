...
/* 
-1에서 시작해서 0까지 다익 0의 좌표 전부 받아서 전처리
군인의 수가 5명 이하니 5!으로 가능
*/
...
void Main(){ 
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < n; j++){
		ri(board[i][j]);
		if(board[i][j] == 0) w.push_back({i, j});
		else if(board[i][j] == -1) {sx = i,sy = j;}
	}
	auto Dijkstra = [&](int sx, int sy) -> vvi{
		vvi dist(n + 1, vi(n + 1, INF));
		min_pq<ti3> PQ; PQ.push({dist[sx][sy] = 0, sx, sy});
		while(PQ.size()){
			...
			for(int d = 0; d < 4; d++){
				...
				auto cost = board[nx][ny] == -1 ? 0 : board[nx][ny];
	...
	vi I(w.size()); iota(all(I), 0);
	auto D = Dijkstra(sx, sy);
	int mn = INF;
	do{
		int ret = D[w[I[0]].first][w[I[0]].second];
		for(int i = 1; i < I.size(); i++){
			auto s = I[i - 1], e = I[i];
			ret += M[s][w[e].first][w[e].second];
		}
		ret += M[I.back()][sx][sy];
		mn = min<int>(mn, ret);
