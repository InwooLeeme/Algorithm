...
void Main(){
    int n,m; while(cin >> n >> m and n){
		int s,e; ri(s, e);
		vector<vp> G(n + 1),revG(n + 1);
		while(m--){
			int a,b,c; ri(a, b, c);
			G[a].push_back({c, b}); revG[b].push_back({c, a});
		}
		auto Dijkstra = [&](vector<vp>& G, int s) -> vi{
			min_pq<pii> PQ;
			vi dist(n + 1, INF); PQ.push({dist[s] = 0, s});
			while(PQ.size()){
		...
		auto dist1 = Dijkstra(G, s); auto dist2 = Dijkstra(revG, e);
		// Dijkstra
		vi dist(n + 1, INF);
		min_pq<pii> PQ; PQ.push({dist[s] = 0, s});
		while(PQ.size()){
			auto [cost, cur] = PQ.top(); PQ.pop();
			if(dist[cur] < cost) continue;
			if(cur == e) break;
			for(auto& [ncost, nxt] : G[cur]){
				if(dist1[e] == ncost + dist2[nxt] + dist1[cur]) continue;
				if(dist[nxt] > ncost + cost) PQ.push({dist[nxt] = ncost + cost, nxt});
			}
		}
		if(dist[e] == INF) po(-1);
