...
void Main(){
	...
	while(q--){
		int t,a,b,c,d,k; ri(t,a,b,c,d); ++a,++b,++c,++d;
		if(t == 1){
			ri(k);
			P[a][b] += k;
			P[a][d + 1] -= k;
		...
		else r1 = a, c1 = b, r2 = c, c2 = d;
	}
	for(int i = 1; i < 1111; i++){
		for(int j = 1; j < 1111; j++) P[i][j] += P[i][j - 1];
	}
	for(int i = 1; i < 1111; i++){
		for(int j = 1; j < 1111; j++) P[i][j] += P[i - 1][j];
	}
	int ret = 0;
	for(int i = r1; i <= r2; i++){
		for(int j = c1; j <= c2; j++) ret += (v[i][j] + P[i][j]);
	}
	po(ret);
}	
