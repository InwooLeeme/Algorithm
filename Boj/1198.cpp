...
void Main(){
	int n; ri(n);
	vp v(n); for(auto& [a, b] : v) ri(a, b);
	vi w;
	for(int i = 0; i < n; i++) w.push_back(i < 3 ? 0 : 1);
	double mx = 0.0;
	do{
		vp c;
		for(int i = 0; i < n; i++){
			if(!w[i]) c.push_back(v[i]);
		}
		mx = max<double>(mx, abs(c[0].first * c[1].second + c[1].first * c[2].second + c[2].first * c[0].second - c[1].first * c[0].second - c[2].first * c[1].second - c[0].first * c[2].second) / 2.0);
