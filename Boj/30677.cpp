...
void Main(){
	ri(n, k, c, r);
	vi ba(k),s(k),p(k); ri(ba); ri(s); ri(p);
	vi cnt(k + 1);
	int combo = 0,ret = 0,tired = 0;
	auto f = [&](int idx){
		return ba[idx] * (1 + ((double)(combo * c) / 100)) * (1 + ((double)(cnt[idx] * s[idx]) / 100));
	};
	for(int i = 0; i < n; i++){
		int l; ri(l);
		if(l){
			...
			combo = 0; tired -= r; tired = max<int>(tired, 0);
		}
	}
	po(ret);
} 
