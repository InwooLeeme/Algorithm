...
void Main(){
	...
	vector<array<int, 3>> D(n + 1, {INF, INF ,0});
	pq.push({D[x][0] = 0, D[x][1] = 0, x}); D[x][2] = 1;
	while(SZ(pq)){
		auto [cdist, shcnt, cur] = pq.top(); pq.pop();
		if(D[cur][0] < cdist or D[cur][1] < shcnt) continue;
		for(const auto& [cost, nxt] : g[cur]){
			auto ncost = cdist + cost;
			auto nshcnt = shcnt + 1;
			if(D[nxt][0] > ncost){
				D[nxt][0] = ncost;
				D[nxt][1] = nshcnt;
				D[nxt][2] = D[cur][2];
				pq.push({D[nxt][0], D[nxt][1], nxt});
			}
			else if(D[nxt][0] == ncost and D[nxt][1] > nshcnt){
				D[nxt][1] = nshcnt;
				D[nxt][2] = D[cur][2];
				pq.push({D[nxt][0], D[nxt][1], nxt});
			}
			else if(D[nxt][0] == ncost and D[nxt][1] == nshcnt){
				D[nxt][2] += D[cur][2]; D[nxt][2] %= MOD;
			}
		}
	}
	if(D[y][0] == INF) po(-1);
	else{ po(D[y][0]); po(D[y][1]); po(D[y][2]); }
}
