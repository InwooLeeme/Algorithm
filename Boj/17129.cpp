...
int32_t main(){
    ...
    rep(i, 0, n){
        rep(j, 0, m){
            if(board[i][j] == '3' and dist[i][j] != -1) mn = min<int>(mn, dist[i][j]);
            else if(board[i][j] == '4' and dist[i][j] != -1) mn = min<int>(mn, dist[i][j]);
            else if(board[i][j] == '5' and dist[i][j] != -1) mn = min<int>(mn, dist[i][j]);
        }
    }
    if(mn == INF) pf1l("NIE");
    else{
