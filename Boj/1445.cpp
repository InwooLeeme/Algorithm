...
void Main(){ 
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < m; j++){
            ...
            if(cur == 'S'){
                dist[i][j] = 0;
                PQ.push({dist[i][j], 0, 0, i, j});
            }
        }
    }
    while(PQ.size()){
        auto [cdist, gstate, gnstate, x, y] = PQ.top(); PQ.pop();
        if(board[x][y] == 'F'){
            po(gstate, gnstate);
            return;
        }
        for(int d = 0; d < 4; d++){
            ...
            if(Bound(nx, ny)) continue;
            auto ngstate = gstate;
            auto ngnstate = gnstate;
            if(board[nx][ny] == '.' and F(nx, ny)) cost = 5000, ngnstate++;
            if(board[nx][ny] == 'g') cost = 10000, ngstate++;
            if(dist[nx][ny] > cdist + cost){
                dist[nx][ny] = cdist + cost;
                PQ.push({dist[nx][ny], ngstate, ngnstate, nx, ny});
            }
        }
    }
} 	
