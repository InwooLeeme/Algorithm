...
void Main(){
	ri(n, m, q);
	vi ret(n); iota(all(ret), 1);
	for(int i = 0; i < q; i++){
		int a,b,c; string s; ri(a, b, c, s); // 경과시간, 팀번호, 문제번호, 결과
		if(s == "AC"){
			if(vist[b][c]) continue;
			P[b]++;
			vist[b][c] = 1;
			tot[b] += (W[b][c] * 20) + a;
			debug(tot[b]);
	...
	sort(all(ret), [&](int& a, int& b){
		return P[a] == P[b] ? tot[a] == tot[b] ? a < b : tot[a] < tot[b] : P[a] > P[b];
	});
