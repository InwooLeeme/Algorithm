...
struct Query{
    ...
    bool operator < (const Query& q) const{
        if(s / sqN != q.s / sqN) return s / sqN < q.s / sqN;
        return e < q.e;
    };
};

int cur;
int cnt[333'333],T[333'333];

void Push(int x){
    if(cnt[x] != 0) T[cnt[x]]--;
    cnt[x]++; T[cnt[x]]++;
    cur = max<int>(cur, cnt[x]);
}

void Pop(int x){
    T[cnt[x]]--;
    if(cnt[x] == cur and !T[cnt[x]]) cur--;
    cnt[x]--;
    T[cnt[x]]++;
}
...
void solve(){
    ...
    int s = 1,e = 0; cur = 0;
    for(int i = 0; i < q; i++){
        while(s < Q[i].s) Pop(v[s++]);
        while(s > Q[i].s) Push(v[--s]);
        while(e < Q[i].e) Push(v[++e]);
        while(e > Q[i].e) Pop(v[e--]);
        ret[Q[i].idx] = cur;
    }
    for(const auto& c : ret) po(c);
}
