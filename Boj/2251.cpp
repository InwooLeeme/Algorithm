...
bool Bound(int x, int y, int z){ return x < 0 or x > A or y < 0 or y > B or z < 0 or z > C; }

pii Move(int a, int b, int la, int lb){
	int m = abs(lb - b);
	if(m >= a) return {0, min<int>(a + b, lb)};
	return {a - m, lb};
}
...
void Main(){
	...
	while(Q.size()){
		...
		if(!x) ret.push_back(z);
		if(x){
			for(int d = 0; d < 2; d++){
				auto nx = x,ny = y,nz = z;
				auto r = Move(nx, (d == 0 ? ny : nz), A, (d == 0 ? B : C));
				nx = r.fi, (d == 0 ? ny : nz) = r.se;
				if(Bound(nx, ny, nz)) continue;
				if(dist.count({nx, ny, nz})) continue;
				dist[{nx, ny, nz}] = dist[{x, y, z}] + 1; Q.push({nx, ny, nz});
			}
		}
		if(y){
			for(int d = 0; d < 2; d++){
				auto nx = x,ny = y,nz = z;
				auto r = Move(ny, (d == 0 ? nx : nz), B, (d == 0 ? A : C));
				ny = r.fi, (d == 0 ? nx : nz) = r.se;
				if(Bound(nx, ny, nz)) continue;
				if(dist.count({nx, ny, nz})) continue;
				dist[{nx, ny, nz}] = dist[{x, y, z}] + 1; Q.push({nx, ny, nz});
			}
		...
		if(z){
			for(int d = 0; d < 2; d++){
				auto nx = x,ny = y,nz = z;
				auto r = Move(nz, (d == 0 ? nx : ny), C, (d == 0 ? A : B));
				nz = r.fi, (d == 0 ? nx : ny) = r.se;
				if(Bound(nx, ny, nz)) continue;
				if(dist.count({nx, ny, nz})) continue;
				dist[{nx, ny, nz}] = dist[{x, y, z}] + 1; Q.push({nx, ny, nz});
			}
