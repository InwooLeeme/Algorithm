...
void Main(){
	ri(N, Q);
	vector<p> I(N); for(auto& [a, b] : I) ri(a, b);
	sort(all(I));
	auto Cmp = [&](const p& a,const p& b) -> bool{ return a.fi < b.fi; };
	while(Q--){
		int u,v,x,y; ri(u, v, x, y);
		int L = lower_bound(all(I), make_pair(u, 0), Cmp) - I.begin();
		int R = upper_bound(all(I), make_pair(v, 0), Cmp) - I.begin();
		int ret = 0;
		for(int i = L; i < R; i++){
			ret += (x <= I[i].se and I[i].se <= y);
		}
		po(ret);
	}
}
