...
void Sol(){
	int n,m; ri(n, m);
	vvi dist(n + 1, vi(n + 1, INF)),dist2(n + 1, vi(n + 1, INF));
	for(int i = 0; i < m; i++){
		int a,b; ri(a, b);
		dist[a][b] = min<int>(dist[a][b], 1);
		dist2[b][a] = min<int>(dist2[b][a], 1);
	}
	// floyd
	auto F = [&]() -> void{
		for(int k = 1; k <= n; k++) for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++){
			dist[i][j] = min<int>(dist[i][j], dist[i][k] + dist[k][j]);
			dist2[i][j] = min<int>(dist2[i][j], dist2[i][k] + dist2[k][j]);
		}
	};
	F();
	int ret = 0,H = (n + 1) >> 1;
