...
int N,M,K,C;
string v[111];
int dist[111][111][111];
using p = array<int, 3>;
const int kdx[5] = {1, 0, -1, 0, 0};
const int kdy[5] = {0, 1, 0, -1, 0};
bool Bound(int x, int y){ return x < 0 or x >= N or y < 0 or y >= M;}

void Main(){
	memset(dist, -1, sizeof dist);
	ri(N, M, K, C); cin.ignore();
	queue<p> Q;
	int ex = -1, ey = -1;
	for(int i = 0; i < N; i++){
		ri(v[i]);
		for(int j = 0; j < M; j++){
			if(v[i][j] == 'S') dist[i][j][0] = 0, Q.push({0, i, j});
	...
	while(Q.size()){
		...
		for(int d = 0; d < 5; d++){
			...
			if(v[nx][ny] == '#') continue;
			auto nH = H;
			if(v[nx][ny] == 'H') nH = max<int>(0, H - K);
			if(v[nx][ny] == '.' or v[nx][ny] == 'E') nH = min<int>(100, H + C);
			if(nH == 100) continue;
			if(dist[nx][ny][nH] != -1) continue;
			dist[nx][ny][nH] = dist[x][y][H] + 1; Q.push({nH, nx, ny});
		}
	}
	int mn = INF;
	for(int i = 0; i < 100; i++){
		if(dist[ex][ey][i] == -1) continue;
		mn = min<int>(mn, dist[ex][ey][i]);
	}
	po(mn == INF ? -1 : mn);
}
