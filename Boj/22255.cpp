...
int n,m,sx,sy,ex,ey;
int v[111][111], dist[111][111][3];
using p = array<int, 4>;
bool Bound(int x, int y){ return x < 0 or x >= n or y < 0 or y >= m; }

void Main(){
	ri(n, m, sx, sy, ex, ey); --sx,--sy,--ex,--ey;
	...
	min_pq<p> PQ;
	dist[sx][sy][1] = 0;
	PQ.push({0, 1, sx, sy}); // cdist, cnt, x, y
	while(PQ.size()){
		auto [cdist, cnt, x, y] = PQ.top(); PQ.pop();
		if(dist[x][y][cnt] < cdist) continue;
		for(int d = 0; d < 4; d++){
			auto ncnt = (cnt + 1) % 3;
			if(cnt == 1 and (~d & 1)) continue;
			if(cnt == 2 and (d & 1)) continue;
			auto nx = x + dx[d], ny = y + dy[d];
			auto ncost = cdist + v[nx][ny];
			if(Bound(nx, ny)) continue;
			if(v[nx][ny] == -1) continue;
			if(dist[nx][ny][ncnt] > ncost){
				dist[nx][ny][ncnt] = ncost;
				PQ.push({dist[nx][ny][ncnt], ncnt, nx, ny});
			}
		}
	}
	int ret = min<int>({dist[ex][ey][0], dist[ex][ey][1], dist[ex][ey][2]});
	po(ret == INF ? -1 : ret);
}
