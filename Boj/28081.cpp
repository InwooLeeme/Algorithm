...
void Main(){
	...
	vi a{0};
	for(int i = 0; i < x; i++){
		int t; ri(t); a.push_back(t);
	}
	a.push_back(m);
	vi l,r;
	for(int i = 1; i < (int)a.size(); i++) l.push_back(a[i] - a[i - 1]);
	ri(x);
	vi b{0};
	for(int i = 0; i < x; i++){
		int t; ri(t); b.push_back(t);
	}
	b.push_back(n);
	for(int i = 1; i < (int)b.size(); i++) r.push_back(b[i] - b[i - 1]);
	int ret = 0;
	asort(l); asort(r);
	for(const auto& i : l){
		int M = upper_bound(all(r), k / i) - r.begin();
		ret += M;
	}
	po(ret);
}	
