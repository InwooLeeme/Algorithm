...
void Main(){
	...
	vvi D(70707, vi(N + 1, INF)); D[0][0] = 0;
	for(int j = 1; j <= N; j++) for(int i = 0; i <= 70000; i++){
		D[i][j] = D[i][j - 1] + b[j];
		if(i - a[j] >= 0) D[i][j] = min<int>(D[i][j], D[i - a[j]][j - 1]);
	}
	int ret = INF;
	for(int i = 0; i <= 70000; i++) ret = min<int>(ret, max<int>(i, D[i][N]));
	po(ret);
}
