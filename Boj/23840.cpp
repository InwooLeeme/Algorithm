...
void Main(){
	...
	vi c{s};
	for(int i = 0; i < p; i++){
		int t; ri(t); c.push_back(t);
	}
	c.push_back(e);
	auto Dijkstra = [&](int st) -> vi{
		vi dist(n + 1, INF);
		...
		while(PQ.size()){
			...
			for(const auto& [cost, nxt] : G[cur]){
				if(dist[nxt] > cdist + cost){
					dist[nxt] = cdist + cost; PQ.push({dist[nxt], nxt});
				}
			}
		}
		return dist;
	...
	vvi w(N, vi(N, INF));
	for(int i = 0; i < N; i++){
		auto dist = Dijkstra(c[i]);
		for(int j = 0; j < N; j++){
			if(i == j) continue;
			w[i][j] = dist[c[j]];
		}
	}
	vvi D(N + 1, vi((1 << (N - 1)) + 1, -1));
	// tsp
	auto TSP = [&](int cur, int vist, auto&& TSP) -> int{
		if(vist == (1 << (N - 1)) - 1) return w[cur][N - 1];
		int& ret = D[cur][vist];
		if(ret != -1) return ret;
		ret = INF;
		for(int i = 0; i < N - 1; i++){
			...
			ret = min<int>(ret, TSP(i, vist | (1 << i), TSP) + w[cur][i]);
		}
		return ret;
	};
	int ret = TSP(0, 1, TSP);
