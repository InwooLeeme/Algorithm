...
bool CanBridge(vi& w, int st, int en){
	for(int nxt = st; nxt < en; nxt++){
		if(Bound(nxt)) return 0;
		if(w[nxt]) return 0;
	}
	return 1;
}

bool IsSame(vi& v, int st, int en){
	int ret = v[st];
	for(int i = st; i < en; i++) if(ret != v[i]) return 0;
	return 1;
}
...
bool Check(vi& v){
	vi vist(111);
	// all road is same?
	if(count(all(v), v[0]) == n) return 1;
	
	// 오르막인지 내리막인지
	if(is_sorted(all(v))){
		int mx = max(v),mn = min(v);
		if(v[0] == mx and v.back() == mn){ // 0을 기준으로 내리막
			for(int i = 1; i < n; i++){
				if(v[i - 1] > v[i]){
					if(abs(v[i] - v[i - 1]) != 1 or !CanBridge(vist, i, i + L) or !IsSame(v, i, i + L)) return 0;
					for(int j = i; j < i + L; j++) vist[j] = 1; // 경사로 공사 [i, i + L)
				}
			}
			return 1;
		}
			...
			for(int i = 1; i < n; i++){
				if(v[i - 1] < v[i]){
					if(abs(v[i] - v[i - 1]) != 1 or !CanBridge(vist, i - L, i) or !IsSame(v, i - L, i)) return 0;
					for(int j = i - L; j < i; j++) vist[j] = 1;
				}
			}
			return 1;
		}
...
void Sol(){
	...
	for(int i = 0; i < n; i++){
		vi R,D;
		for(int j = 0; j < n; j++) R.push_back(board[i][j]); // right
		for(int j = 0; j < n; j++) D.push_back(board[j][i]); // down
		ret += Check(R);
		ret += Check(D);
	}
	po(ret);
...
void Input(){
	ri(n, L); 
	fup(i, 0, n - 1, 1) fup(j, 0, n - 1, 1) ri(board[i][j]);
}
