...
void Main(){
  ri(N, M, s, e);
  Dinic<int> flow(N << 1 | 1);
  for(int i = 1; i <= N; i++){
    int x; ri(x);
    flow.add_edge(in(i), out(i), x);
  }
  while(M--){
    int a,b; ri(a, b);
    flow.add_edge(out(a), in(b), INF);
    flow.add_edge(out(b), in(a), INF);
  }
  auto [cost, l, r, cut] = flow.minimum_cut(in(s), out(e));
  queue<int> Q; Q.push(s); D[s] = 1;
  while(SZ(Q)){
    auto cur = Q.front(); Q.pop();
    for(const auto &e : flow.g[cur]){
      if(D[e.v]) continue;
      if(e.c - e.f > 0){
        D[e.v] = 1;
        Q.push(e.v);
  ...
  vi ret;
  for(int i = 1; i <= N; i++) if(D[i] and !D[out(i)]) ret.pb(i);
  po(ret);
}
