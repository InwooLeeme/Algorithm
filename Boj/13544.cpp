...
struct MergeSortTree{
    ...
    int Query(int L,int R,int K,int nodeNum, int nodeL,int nodeR){
        if(R < nodeL || nodeR < L) return 0;
        if(L <= nodeL && nodeR <= R) return tree[nodeNum].end() - upper_bound(tree[nodeNum].begin(), tree[nodeNum].end(), K);
...
int32_t main(){
    ...
    while(Q--){
        int i,j,k; sf3(i, j, k);
        ret = ST.Query(ret ^ i, ret ^ j, ret ^ k);
        pf1l(ret);
    }
}