...
void Sol(){
	int n,m,k; ri(n, m, k); ++k;
	auto Bound = [&](int x, int y) -> bool{ return x < 0 or x >= n or y < 0 or y >= m; };
	vs v(n); ri(v);
	min_pq<ti3> PQ; vvi dist(n + 1, vi(m + 1, INF));
	[&](){
		for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
			if(v[i][j] == 'S'){
				dist[i][j] = 0; PQ.push({dist[i][j], i, j}); return;
			}
		}
	}();
	while(PQ.size()){
		...
		for(int d = 0; d < 4; d++){
			...
			if(dist[nx][ny] > cdist + (v[nx][ny] == '@' ? k : 1)){
				dist[nx][ny] = cdist + (v[nx][ny] == '@' ? k : 1);
	...
	int mn = INF;
	for(int i = 0; i < m; i++){
		if(v[0][i] != '#') mn = min<int>(mn, dist[0][i]);
		if(v[n - 1][i] != '#') mn = min<int>(mn, dist[n - 1][i]);
	}
	for(int i = 0; i < n; i++){
		if(v[i][0] != '#') mn = min<int>(mn, dist[i][0]);
		if(v[i][m - 1] != '#') mn = min<int>(mn, dist[i][m - 1]);
	}
	po(mn + 1);
}
