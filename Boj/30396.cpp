...
bool bound(int x, int y){ return x < 0 or x >= 4 or y < 0 or y >= 4; }

void solve(){
	vs a(4),b(4); ri(a); ri(b);
	queue<vs> q;
	map<vs, int> dist; dist[a] = 0; q.push(a);
	while(si(q)){
		...
		for(int x = 0; x < 4; x++){
			for(int y = 0; y < 4; y++){
				...
				for(int d = 0; d < 8; d++){
					auto nxtx = x + nx[d], nxty = y + ny[d];
					auto nxt = cur;
					if(bound(nxtx, nxty)) continue;
					if(nxt[nxtx][nxty] == '1') continue;
					nxt[x][y] = '0'; nxt[nxtx][nxty] = '1';
