...
int n,m,mx;
int I[222][222], P[222][222];

void Main(){
    ri(n, m);
    for(int i = 1; i <= n; i++){
        string s; ri(s);
        for(int j = 1; j <= m; j++) I[i][j] = s[j - 1] - '0';
    }
    for(int j = 1; j <= m; j++){
        for(int i = 1; i <= n; i++) P[i][j] = P[i - 1][j] + I[i][j];
    }
    for(int i = 1; i <= n; i++){
        for(int j = 1; j <= m; j++) P[i][j] += P[i][j - 1];
    }
    for(int r1 = 1; r1 <= n; r1++){
        for(int c1 = 1; c1 <= m; c1++){
            for(int r2 = r1; r2 <= n; r2++){
                for(int c2 = c1; c2 <= m; c2++){
                    auto cur = P[r2][c2] - P[r1 - 1][c2] - P[r2][c1 - 1] + P[r1 - 1][c1 - 1];
                    if(cur == (r2 - r1 + 1) * (c2 - c1 + 1)) mx = max<int>(mx, cur);
                }
            }
        }
    }
