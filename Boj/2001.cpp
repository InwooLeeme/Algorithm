...
void solve(){
	...
	while(si(q)){
		...
		for(const auto& [lim, nxt] : g[cur]){
			for(int i = 0; i < 2; i++){
				auto nstate = ((i and v[nxt] != -1) ? (state | (1 << v[nxt])) : state);
				auto cnt = __builtin_popcount(state);
				if(cnt > lim or dist[nxt][nstate] != -1) continue;
				dist[nxt][nstate] = dist[cur][state] + 1; q.push({nstate, nxt});
			}
		}
	}
	int res = 0;
	for(int i = 0; i <= (1 << k); i++){
		if(dist[1][i] != -1) res = max<int>(res, __builtin_popcount(i));
	}
	po(res);
}
