...
void solve(){
    ri(n, m);
    for(int i = 0; i < m; i++){
        int s,e, a,b,c; ri(s, e, a, b, c);
        auto co = Conv(a, b, c);
        g[s].pb({co, e}); 
    }
    minpq<ti3> pq;
    vi D(n + 1, INF),Cnt(n + 1, INF);
    pq.push({D[1] = 0, Cnt[1] = 1, 1});
    while(si(pq)){
        auto [cdist, cnt, cur] = pq.top(); pq.pop();
        if(D[cur] < cdist) continue;
        for(const auto& [cost, nxt] : g[cur]){
            if(D[nxt] > cdist + cost){
                Cnt[nxt] = cnt + 1;
                pq.push({D[nxt] = cdist + cost, Cnt[nxt], nxt});
            }
            else if(D[nxt] == cdist + cost and Cnt[nxt] > cnt + 1){
                Cnt[nxt] = min<int>(Cnt[nxt], cnt + 1);
                pq.push({D[nxt] = cdist + cost, Cnt[nxt], nxt});
            }
        }
    }
    if(D[n] == INF) po("It is not a great way.");
    else po(D[n], Cnt[n]);
}
