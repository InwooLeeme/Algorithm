...
void DFS(int x, int y, int color){
	...
	for(int d = 0; d < 4; d++){
		...
		if(Bound(nx, ny) or vist[nx][ny] or v[nx][ny] != color) continue;
		DFS(nx, ny, color);
	}
}
...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++) ri(B[i][j]);
	// Conv
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
		if(v[i][j] != B[i][j]) diff[i][j] = 1;
	}
	[&](){
		for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
			if(!vist[i][j] and diff[i][j]){
				DFS(i, j, v[i][j]);
				return;
			}
		}
