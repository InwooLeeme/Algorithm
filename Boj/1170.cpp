...
int n,m, D[111111], par[111111];
int dfsn,dfs_n[111111];
vector<int> G[111111];
...
void DFS1(int cur, int prv){
    ...
    for(auto& nxt : G[cur]){
        if(nxt == prv) continue;
        if(!dfs_n[nxt]) par[nxt] = cur, DFS1(nxt, cur);
        else if(dfs_n[nxt] < dfs_n[cur]) D[cur]++,D[par[nxt]]--;
    }
}
...
int DFS3(int cur){
	if(D[cur] > 1) return 1;
	for(auto& nxt : G[cur]) if(par[nxt] == cur and DFS3(nxt)) return 1;
	return 0;
}
