...
void Main(){
    ...
    for(int i = 0; i < n; i++) for(int j = 0; j < n; j++) ri(v[i][j]), mx = max<int>(mx, v[i][j]), I.pb(v[i][j]);
    auto Check = [&](const int L, const int R) -> bool{
        if(!(L <= v[r][c] and v[r][c] <= R)) return 0;
        int C = 0;
        memset(D, -1, sizeof(D));
        queue<pii> Q; Q.push({r, c}); D[r][c] = 0;
        while(SZ(Q)){
    ...
    for(auto c : I){
        ...
        while(lo + 1 < hi){
            int mid = lo + hi >> 1;
            if(!Check(c, mid)) lo = mid;
            else hi = mid;
        }
        if(hi != mx + 1) mn = min<int>(mn, hi - c);
    }
    po(mn);
}
