...
int c2i(int x, int y){ return 505 * x + y; }
...
void Main(){ 
	ri(n, m);
	for(int i = 1; i <= n; i++) for(int j = 1; j <= m; j++) ri(v[i][j]);
	vi dist(303030, INF);
	// Graph Construct
	for(int i = 1; i <= n; i++) for(int j = 1; j <= m; j++){
		if(v[i][j] == '/'){
			AddEdge(c2i(i, j), c2i(i - 1,j - 1), 1);
			AddEdge(c2i(i, j - 1), c2i(i - 1, j), 0);
		}
		else{
			AddEdge(c2i(i - 1, j - 1), c2i(i, j), 0);
			AddEdge(c2i(i - 1, j), c2i(i, j - 1), 1);
		}
	}
	int st = c2i(0, 0);
	min_pq<pii> PQ; PQ.push({dist[st] = 0, st});
	while(PQ.size()){
