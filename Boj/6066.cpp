...
void Main(){
    int n,m; ri(n, m);
    constexpr int INF = int(2e18);
    const int MX = 50000 * 100 + 1;
    vector<int> d(MX, INF),w(n),v(n);
    for(int i = 0; i < n; i++) ri(w[i], v[i]);
    d[0] = 0;
    for(int i = 0; i < n; i++){
        for(int j = w[i]; j < MX; j++) d[j] = min<int>(d[j], d[j - w[i]] + v[i]);
    }
    int mn = INF;
    for(int i = m; i < MX; i++) mn = min<int>(mn, d[i]);    
    po(mn);
}
