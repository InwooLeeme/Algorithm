...
struct SegTree{
	...
	SegTree(){
		for(int i = 0; i < 2'0000'20; i++) lazy[i][0] = 1, lazy[i][1] = 0;
	}

	void Push(int node, int s, int e){
		if(lazy[node][0] == 1 and lazy[node][1] == 0) return;
		if(s != e){
			for(const auto& nxt : { node << 1, node << 1 | 1}){
				lazy[nxt][0] *= lazy[node][0]; lazy[nxt][0] %= MOD;
				lazy[nxt][1] *= lazy[node][0]; lazy[nxt][1] %= MOD;
				lazy[nxt][1] += lazy[node][1]; lazy[nxt][1] %= MOD;
			}
		}
		tree[node] = lazy[node][0] * tree[node] + lazy[node][1] * (e - s + 1); tree[node] %= MOD;
		lazy[node][0] = 1, lazy[node][1] = 0;
	}

	void Update(int l, int r, ll mul, ll sum, int node = 1, int s = 1, int e = n){
		Push(node, s, e);
		...
		int m = s + e >> 1;
		Update(l, r, mul, sum, node << 1, s, m);
		Update(l, r, mul, sum, node << 1 | 1, m + 1, e);
	...
	ll Query(int l, int r, int node = 1, int s = 1, int e = n){
		...
		if(l <= s and e <= r) return tree[node] % MOD;
		int m = s + e >> 1;
		return (Query(l, r, node << 1, s, m) + Query(l, r, node << 1 | 1, m + 1, e)) % MOD;
	}
} ST;

int top[505050], sz[505050], dep[505050], par[505050], in[505050], out[505050];
vector<int> inp[505050], G[505050];
int pv;
bool vist[505050];
...
void SubUpdate(int x, ll mul, ll sum){
	ST.Update(in[x], out[x], mul, sum);
}

void PathUpdate(int a, int b, ll mul, ll sum){
	while(top[a] != top[b]){
		if(dep[top[a]] < dep[top[b]]) swap(a, b);
		int st = top[a];
		ST.Update(in[st], in[a], mul, sum);
		a = par[st];
	}
	if(dep[a] > dep[b]) swap(a, b);
	ST.Update(in[a], in[b], mul, sum);
}

ll SubQuery(int x){
	return ST.Query(in[x], out[x]);
}
...
ll PathQuery(int a, int b){
	...
	while(top[a] != top[b]){
		if(dep[top[a]] < dep[top[b]]) swap(a, b);
		int st = top[a];
		ret += ST.Query(in[st], in[a]); ret %= MOD;
		a = par[st];
	}
	if(dep[a] > dep[b]) swap(a, b);
	ret += ST.Query(in[a], in[b]); ret %= MOD;
	return ret;
}
...
void Main(){
	...
	for(int i = 1; i < n; i++){
		int a,b; ri(a, b);
		inp[a].push_back(b); inp[b].push_back(a);
	}
	DFS(); DFS1(); DFS2();
	while(q--){
		int t,x,y,v; ri(t);
