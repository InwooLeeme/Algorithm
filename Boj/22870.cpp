...
int n, m, s, e;
vector<pii> G[200'001];
vector<int> D(222'222);
bitset<200'001> B, C;

int Dijkstra(int st, int en) {
	fill(D.begin(), D.end(), INF);
	min_pq<pii> PQ; PQ.push({ D[st] = 0, st });
	while (PQ.size()) {
		auto [cdist, cur] = PQ.top(); PQ.pop();
		if (D[cur] < cdist) continue;
		if (cur == en) break;
		for (auto [cost, nxt] : G[cur]) {
			if (D[nxt] > cdist + cost && !B[nxt])
				PQ.push({ D[nxt] = cdist + cost, nxt });
		}
	}
	return D[en];
}
...
void BFS(int st, int en) {
	queue<int> Q; Q.push(en), C[en] = 1;
	while (Q.size()) {
		auto cur = Q.front(); Q.pop();
		for (auto [cost, nxt] : G[cur]) {
			if (D[nxt] + cost == D[cur] && !C[nxt])
				Q.push(nxt), C[nxt] = 1;
		}
	}
}
...
void Check(int st, int en) {
	for (int cur = st; cur != en;) {
		int mn = 1e9;
		for (auto [cost, nxt] : G[cur]) {
			if (D[cur] + cost == D[nxt] && C[nxt])
				mn = min(mn, nxt);
		}
		B[cur = mn] = 1;
	}
...
void Main(){
	...
	ri(s, e);
	auto L = Dijkstra(s, e); BFS(s, e); Check(s, e);
	auto R = Dijkstra(e, s);
	po(L + R);
}
