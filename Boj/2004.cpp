...
pii f(int x){
	int l = 0,r = 0;
	for(int i = 2; i <= x; i *= 2) l += (x / i);
	for(int i = 5; i <= x; i *= 5) r += (x / i);
	return {l, r};
}

void Main(){
	int n,r; ri(n, r);
	auto [ntcnt, nfcnt] = f(n);
	auto [rtcnt, rfcnt] = f(r);
	auto [nrtcnt, nrfcnt] = f(n - r);
	po(min<int>(ntcnt - rtcnt - nrtcnt, nfcnt - rfcnt - nrfcnt));
} 
