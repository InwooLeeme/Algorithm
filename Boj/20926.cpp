...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
		if(isdigit(v[i][j])) C[i][j] = (v[i][j] - '0');
		if(v[i][j] == 'T') x1 = i,y1 = j;
	}
	min_pq<ti3> PQ; 
	PQ.push({dist[x1][y1] = 0, x1, y1});
	while(PQ.size()){
		...
		for(int d = 0; d < 4; d++){
			auto nx = x, ny = y;
			int cost = 0;
			auto fx = dx[d], fy = dy[d];
			while(!Bound(nx + fx, ny + fy) and isdigit(v[nx + fx][ny + fy])){ nx += fx, ny += fy; cost += C[nx][ny]; }
			if(Bound(nx + fx, ny + fy) or v[nx + fx][ny + fy] == 'H') continue;
			if(v[nx + fx][ny + fy] == 'E') nx += fx, ny += fy;
			if(dist[nx][ny] > dist[x][y] + cost){
				dist[nx][ny] = dist[x][y] + cost; PQ.push({dist[nx][ny], nx, ny});
			}
		}
	}
	po(-1);
