...
int n,m,w,h;
int v[2222][2222],u[2222][2222],P[2222][2222];
...
void BFS(int x, int y){
	...
	while(Q.size()){
		...
		for(int d = 0; d < 4; d++){
			...
			if(Bound(nx, ny) or u[nx][ny]) continue;
			if(v[nx][ny] < v[x][y]) continue;
			u[nx][ny] = 1; Q.push({nx, ny});
		}
	}
}
...
void Main(){
	ri(n, m, h, w);
	rep(i, 1, n + 1) rep(j, 1, m + 1) ri(v[i][j]);
	int k; ri(k);
	while(k--){
		int a,b; ri(a, b);
		BFS(a, b);
	}
	rep(i, 1, n + 1) rep(j, 1, m + 1) u[i][j] = (u[i][j] ? 0 : 1);
	rep(i, 1, n + 1) rep(j, 1, m + 1){
		P[i][j] = P[i - 1][j] + P[i][j - 1] - P[i - 1][j - 1] + u[i][j];
	}
	int ret = 0;
	for(int i = 1; i <= n - h + 1; i++) for(int j = 1; j <= m - w + 1; j++){
		if((P[i + h - 1][j + w - 1] - P[i + h - 1][j - 1] - P[i - 1][j + w - 1] + P[i - 1][j - 1]) == 0) ret++;
	}
	po(ret);
}
