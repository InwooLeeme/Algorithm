...
void Main(){
  ri(n);
  min_pq<ti3> pq;
  vi D(n + 1, INF), W(n + 1, INF);
  pq.push({D[0] = 0, W[0] = 0, 0});
  while(SZ(pq)){
    auto [cdist, w, cur] = pq.top(); pq.pop();
    if(D[cur] < cdist) continue;
    if(cur == n){
      po(cdist, w);
      return;
    }
    for(const auto& nxt : {cur + 1, cur * 3, cur * cur}){
      if(nxt > n) continue;
      auto ndist = cdist + 1;
      auto nwater = w + (nxt == (cur + 1) ? 1 : (nxt == cur * 3) ? 3 : 5);
      if(D[nxt] > ndist){
        D[nxt] = ndist;
        W[nxt] = nwater;
        pq.push({D[nxt], W[nxt], nxt});
      }
      else if(D[nxt] == ndist and W[nxt] > nwater){
        W[nxt] = nwater;
        pq.push({D[nxt], W[nxt], nxt});
      }
    }
