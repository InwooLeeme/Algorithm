...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
		...
		auto curd = board[i][j];
		for(int d = 0; d < 4; d++){
			auto nx = i + kdx[(d + curd) % 4], ny = j + kdy[(d + curd) % 4];
			if(Bound(nx, ny)) continue;
			G[i][j].push_back({nx, ny, 0, d});
		}
		// L
		for(int d = 0; d < 4; d++){
			auto nx = i + kdx[(curd - d + 4) % 4], ny = j + kdy[(curd - d + 4) % 4];
			if(Bound(nx, ny)) continue;
			G[i][j].push_back({nx, ny, d, 0});
		}
	}

	min_pq<I> PQ;
	dist[0][0][0] = 0; PQ.push({0, 0, 0, 0}); // r, l, x, y
	while(PQ.size()){
		auto [l, r, x, y] = PQ.top(); PQ.pop();
		if(dist[x][y][l] < r) continue;
		if(x == n - 1 and y == m - 1){
			po("Yes");
			return;
		}
		for(const auto& [nx, ny, lcost, rcost] : G[x][y]){
			if(l + lcost > k or r + rcost > k) continue;
			if(dist[nx][ny][l + lcost] > r + rcost){
				dist[nx][ny][l + lcost] = r + rcost;
				PQ.push({l + lcost, r + rcost, nx, ny});
			}
		}
	}
	po("No");
