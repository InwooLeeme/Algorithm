...
int n;
int v[1010][1010],D[1010][1010][3];

int Sol(int cur, int x, int y){
	if(x == n or y == n) return 0;
	int& ret = D[x][y][cur];
	if(ret != -1) return ret; 
	for(int d = 1; d < 3; d++){
		auto nx = x + dx[d], ny = y + dy[d];
		if(v[x][y] == cur) ret = max<int>({ret, Sol(cur, nx, ny), Sol((cur + 1) % 3, nx, ny) + 1});
		else ret = max<int>(ret, Sol(cur, nx, ny));
	}
	return ret;
}
