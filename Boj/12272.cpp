...
void Sol(){
	...
	while(PQ.size()){
		...
		for(int d = 0; d < 4; d++){
			...
			if(Bound(nx, ny) or v[nx][ny] == -1) continue;
			if(dist[nx][ny] > cdist + 1){
				PQ.push({dist[nx][ny] = cdist + 1, w + v[nx][ny], nx, ny});
			}
			else if(dist[nx][ny] == cdist + 1){
				if(w < w + v[nx][ny]){
					PQ.push({dist[nx][ny] = cdist + 1, w + v[nx][ny], nx, ny});
				}
