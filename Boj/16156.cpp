...
int v[555][555];
vvi dist(555, vi(555, INF));

void Main(){
	ri(n, m);
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++) ri(v[i][j]);
	min_pq<tuple<int, int, int, int>> PQ;
	for(int i = 0; i < n; i++) dist[i][m - 1] = v[i][m - 1], PQ.push({dist[i][m - 1], i, m - 1, i});
	vector<int> ret(n);
	while(PQ.size()){
		auto [cdist, x, y, s] = PQ.top(); PQ.pop();
		...
		for(int d = 0; d < 4; d++){
			...
			if(dist[nx][ny] > cdist + v[nx][ny]){
				dist[nx][ny] = cdist + v[nx][ny];
				PQ.push({dist[nx][ny], nx, ny, s});
			}
		}
	}
