...
bool Bound(int x, int y){ return x < 0 or x >= 3 or y < 0 or y >= 3; }

void f(int dep, int x, int y){
    if(dep == n){
        string cur = "";
		for(const auto& [a, b] : v) cur += board[a][b];
		auto conv = EV.calc_infix(cur);
		if(conv == m){
			po(1);
			for(const auto& [a, b] : v) po(a, b);
			exit(0);
		}
        return;
    }
    for(int d = 0; d < 4; d++){
        ...
        vist[nx][ny] = 1;
		v.push_back({nx, ny});
		f(dep + (string("+-").find(board[nx][ny]) == -1 ? 1 : 0), nx, ny);
		v.pop_back();
		vist[nx][ny] = 0;
    }
}
