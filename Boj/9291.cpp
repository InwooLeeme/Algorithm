...
bool Check(wector<int>& board, int stx,int sty){
    map<int,int> M;
...
bool Sol(){
    wector<int> board(9, vector<int>(9));
    EACH(i , board) EACH(j, i) sf1(j);
    for(int i = 0; i < 9; i++){
        map<int,int> M;
        for(int j = 0; j < 9; j++){
            M[board[i][j]]++;
        }
        if(M.size() != 9) return 0;
    }
    for(int i = 0; i < 9; i++){
        map<int,int> M;
        for(int j = 0; j < 9; j++){
...
int32_t main(){
    ...
    rep(i, 1, t + 1){
        auto ret = Sol();
        cout << "Case " << i << ": " << (ret ? "CORRECT" : "INCORRECT") << "\n";
    }
}