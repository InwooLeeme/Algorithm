...
bool A[1030][1030], B[1030][1030];

void Main(){
	Fenwick2D<int> FTA, FTB;
	FTA.init(1030, 1030); FTB.init(1030, 1030);
	for(int i = 1; i <= 1024; i++){
		for(int j = 1; j <= 1024; j++){
			(~(i + j) & 1 ? FTA : FTB).update(i, j, 1);
			(~(i + j) & 1 ? A : B)[i][j] = 1;
		}
	}
	int Q; ri(Q);
	while(Q--){
		char c; ri(c);
		if(c == 'R'){
			int a,b,c,d; ri(a, b, c, d); --a,--b;
			auto l = FTA.get(c, d) - FTA.get(a, d) - FTA.get(c, b) + FTA.get(a, b);
			auto r = FTB.get(c, d) - FTB.get(a, d) - FTB.get(c, b) + FTB.get(a, b);
			po(l, r);
		}
		else if(c == 'A'){
			int a,b; ri(a, b);
			if(A[a][b]) continue;
			if(B[a][b]) B[a][b] = 0,FTB.update(a, b, -1);
			FTA.update(a, b, 1);
			A[a][b] = 1;
		}
		else if(c == 'B'){
			int a,b; ri(a, b);
			if(B[a][b]) continue;
			if(A[a][b]) A[a][b] = 0,FTA.update(a, b, -1);
			FTB.update(a, b, 1);
			B[a][b] = 1;
		}
	}
