...
void Main(){
	int n,m,k; ri(n, m);
    for(int i = 1; i <= n; i++) ri(A[i]), S.insert({A[i], i});
    ri(k);
    for(int i = 1; i <= k; i++){
        int a,b,c; ri(a, b, c);
        G[a].pb({b, c}); 
    ...
    while(m--){
        ...
        ret = max<int>(ret, cost);
        for(const auto& [nxt, dcost] : G[cur]){
            auto it = S.find({A[nxt], nxt});
            if(it != S.end() and it->se == nxt){
                A[nxt] -= dcost;
                S.erase(it); S.insert({A[nxt], nxt});
            }
        }
    }
    po(ret);
