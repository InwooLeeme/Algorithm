...
void solve(){
	...
	for(int x = 0; x < n; x++) for(int y = 0; y < n; y++){
		if(v[x][y] == '0' or dist[x][y] == -1) continue;
		for(int d = 0; d < 8; d++){
			auto nx = x + ddx[d], ny = y + ddy[d];
			if(bound(nx, ny) or v[nx][ny] == '0' or dist[nx][ny] == -1) continue;
			auto cost = max<int>(dist[x][y], dist[nx][ny]);
			if(I(nx, ny) == n*n - 1 or (I(x, y) == 0) or (I(x, y) == n*n - 1) or (I(nx, ny) == 0)) cost = 0;
			auto a = I(x, y), b = I(nx, ny);
			g[a].pb({cost, b}); g[b].pb({cost, a});
		}
	}
	int S = 0, E = I(n - 1, n - 1);
	vi D(n*n + 1, INF);
	D[S] = 0; minpq<pii> pq; pq.push({D[S] = 0, S});
	while(si(pq)){
		auto [cdist, cur] = pq.top(); pq.pop();
		if(D[cur] < cdist) continue;
		for(const auto& [cost, nxt] : g[cur]){
			auto ncost = max<int>(cdist, cost);
			if(D[nxt] > ncost){
				D[nxt] = ncost;
				pq.push({D[nxt], nxt});
			}
