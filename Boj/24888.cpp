...
int n,m;
vp adj[202020];
int indegree[202020], D[202020], Path[202020];
bitset<202020> w;
vi dist(202020, INF);
...
void Dijkstra(int st){
	...
	while(PQ.size()){
		...
		for(const auto& [cost, nxt] : adj[cur]){
...
void GetDAG(int cur = 1){
	for(const auto& [cost, nxt] : adj[cur]){
		if(dist[cur] + cost != dist[nxt]) continue;
		if(!indegree[nxt]++) GetDAG(nxt);
	}
}
...
void Main(){ 
	...
	if(D[n] != w.count()) po(-1);
		...
		ret.push_back(1); reverse(all(ret));
		po(ret.size());
		po(ret);
	}
} 
