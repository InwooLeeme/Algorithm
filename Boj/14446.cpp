...
void build(){
    for(int i = sz - 1; i; i--){
        mst[i].resize(SZ(mst[i << 1]) + SZ(mst[i << 1 | 1]));
        merge(all(mst[i << 1]), all(mst[i << 1 | 1]), mst[i].begin());
    }
}
...
int Query(int l, int r, int k){
    ...
    l |= sz; r |= sz;
    while(l <= r){
        if(l & 1) res += mst[l].end() - upper_bound(all(mst[l]), k), l++;
        if(~r & 1) res += mst[r].end() - upper_bound(all(mst[r]), k), r--;
        l >>= 1, r >>= 1;
    }
    return res;
}
