...
void Sol(){
	int n,m; ri(n, m);
	vi A(n), B(m); ri(A); ri(B);
	asort(B);
	int ret = 0;
	for(int i = 0; i < n; i++){
		int pos = lower_bound(all(B), A[i]) - B.begin();
		int diff1 = INF,mn1 = INF, diff2 = INF, mn2 = INF;
		if(pos > 0){
			diff1 = abs(A[i] - B[pos - 1]);
			mn1 = min<int>(mn1, B[pos - 1]);
		}
		if(pos < B.size()){
			diff2 = abs(A[i] - B[pos]);
			mn2 = min<int>(mn2, B[pos]);
		}
		if(diff1 == diff2) ret += min<int>(mn1, mn2);
		else ret += (diff1 > diff2 ? mn2 : mn1);
	}
	po(ret);
}
