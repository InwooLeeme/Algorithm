...
void Main(){
	int n,m,L; ri(n, m, L);
	vi check, cod(n),ret;
	for(int i = 0; i < n; i++) check.push_back(i < m ? 0 : 1);
	ri(cod);
	int mn = INF;
	do{
		vi v, w;
		int sum = 0;
		for(int i = 0; i < n; i++) (!check[i] ? v : w).push_back(i);
		for(auto& i : w){
			int mnd = INF;
			for(auto& j : v) mnd = min<int>(mnd, min<int>(abs(cod[j] - cod[i]), L - abs(cod[j] - cod[i])));
			sum += mnd;
		}
		if(mn > sum){
			mn = sum;
