...
void Main(){ 
	...
	for(int i = 1; i <= m; i++){
		int a,b,c; ri(a, b, c);
		G[a][0].push_back({c, b}); G[b][0].push_back({c, a});
		E.push_back({c, a, b}); 
	}
	ri(k);
	for(int i = 1; i <= k; i++){
		for(int j = 0; j < m; j++){
			int x; ri(x);
			auto [c,a,b] = E[j];
			G[a][i].push_back({x, b}); G[b][i].push_back({x, a});
		}
	}
	// Dijkstra
	vvi dist(n + 1, vi(k + 1, INF));
	min_pq<ti3> PQ; PQ.push({dist[s][0] = 0, 0, s});
	while(PQ.size()){
		auto [cdist, state, cur] = PQ.top(); PQ.pop();
		if(dist[cur][state] < cdist) continue;
		for(int i = state; i <= k; i++){
			for(const auto& [cost, nxt] : G[cur][i]){
				if(dist[nxt][i] > cost + cdist){
					dist[nxt][i] = cdist + cost;
					PQ.push({dist[nxt][i], i, nxt});
				}
			}
		}
	}
	int mn = INF;
	for(int i = 0; i <= k; i++) { mn = min<int>(mn, dist[e][i]); }
	po(mn);
} 
