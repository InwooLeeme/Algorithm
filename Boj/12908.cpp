...
void AddEdge(pii a, pii b){
	auto D = abs(a.first - b.first) + abs(a.second - b.second);
	G[M[a]].push_back({D, M[b]});
	G[M[b]].push_back({D, M[a]});
}

void Main(){
	pii s,e;
	ri(s.first, s.second, e.first, e.second);
	M[s] = 1; M[e] = 2;
	int d = abs(s.first - e.first) + abs(s.second - e.second);
	G[1].push_back({d, 2}); G[2].push_back({d, 1});
	int idx = 3;
	vp w;
	for(int i = 0; i < 3; i++){
		pii a, b; ri(a.first, a.second, b.first, b.second);
		w.push_back(a);
		w.push_back(b);
		if(!M.count(a)) M[a] = idx++;
		if(!M.count(b)) M[b] = idx++;
		G[M[a]].push_back({10, M[b]});
		G[M[b]].push_back({10, M[a]});
		AddEdge(s, a);
		AddEdge(s, b);
		AddEdge(e, a);
		AddEdge(e, b);
	...
	vi dist(22, INF); dist[1] = 0;
	min_pq<pii> PQ; PQ.push({dist[1], 1});
	while(PQ.size()){
