...
void Main(){ 
	...
	auto SUM = [&]() -> int{ return accumulate(DQ.begin(), DQ.end(), 0); };
	auto Clear = [&]() -> bool{
		for(const auto& i : DQ) if(i) return 0;
		return 1;
	};
	int ret = 0;
	while(1){
		if(v.empty() and Clear()) break;
		if(SUM() + v.front() <= l and !v.empty()){
			DQ.pop_front(); DQ.push_back(v.front()); v.pop_front();
		}
		else{
			DQ.pop_front(); 
			if(SUM() + v.front() <= l and !v.empty()){
				DQ.push_back(v.front()); v.pop_front();
			}
			else DQ.push_back(0);
		}
		ret++;
