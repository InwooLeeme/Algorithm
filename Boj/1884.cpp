...
void Main(){
	...
	min_pq<ti3> PQ; PQ.push({D[1][0] = 0, 0, 1});
	while(PQ.size()){
		auto [cdist, M, cur] = PQ.top(); PQ.pop();
		if(D[cur][M] < cdist) continue;
		for(const auto& [cost, nM, nxt] : G[cur]){
			auto nxtM = M + nM;
			if(nxtM > k) continue;
			if(D[nxt][nxtM] > cost + cdist){
				D[nxt][nxtM] = cost + cdist;
				PQ.push({D[nxt][nxtM], nxtM, nxt});
			}
		}
	}
	int mn = INF;
	for(int l = 0; l <= k; l++) mn = min<int>(mn, D[n][l]);
	po(mn == INF ? -1 : mn);
}
