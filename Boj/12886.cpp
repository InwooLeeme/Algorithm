...
void Main(){
	...
	while(Q.size()){
		...
		if(a != b){
			if(a > b and !dist.count({a - b, b + b, c})){
				dist[{a - b, b + b, c}] = 1; Q.push({a - b, b + b, c});
			}
			if(b > a and !dist.count({a + a, b - a, c})){
				dist[{a + a, b - a, c}] = 1; Q.push({a + a, b - a, c});
			}
		}
		if(a != c){
			if(a > c and !dist.count({a - c, b, c + c})){
				dist[{a - c, b, c + c}] = 1; Q.push({a - c, b, c + c});
			}
			if(c > a and !dist.count({a + a, b, c - a})){
				dist[{a + a, b, c - a}] = 1; Q.push({a + a, b, c - a});
			}
		}
		if(b != c){
			if(b > c and !dist.count({a, b - c, c + c})){
				dist[{a, b - c, c + c}] = 1; Q.push({a, b - c, c + c});
			}
			if(c > b and !dist.count({a, b + b, c - b})){
				dist[{a, b + b, c - b}] = 1; Q.push({a, b + b, c - b});
			}
		}
	}
	po(0);
