...
int DFS(int cur){
	...
	for(const auto& nxt : G[cur]){
		if(B[nxt] == -1 or (!vist[B[nxt]] and DFS(B[nxt]))){
			A[cur] = nxt;
			B[nxt] = cur;
			return 1;
		}
...
void Main(){
	ri(n, m);
	G.resize(n + 1);
	vi v(n + 1), w(m + 1),l(n + 1),r(m + 1);
	for(int i = 1; i <= n; i++) ri(v[i]);
	for(int i = 1; i <= m; i++) ri(w[i]);
	for(int i = 1; i <= n; i++) ri(l[i]);
	for(int i = 1; i <= m; i++) ri(r[i]);
	for(int i = 1; i <= n; i++){
		for(int j = 1; j <= m; j++){
			if(v[i] > r[j] and w[j] < l[i]){
				G[i].push_back(j);
			}
		}
	}
