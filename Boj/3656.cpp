...
auto Sol = [](){
  ...
  int S = 0, T = n * m + 10,ret = 0;
  Dinic<int> flow(T + 10);
  for(int x = 0; x < n; x++) for(int y = 0; y < m; y++){
    if(x == 0 or y == 0 or x == n - 1 or y == m - 1){
      if(v[x][y] == '.') v[x][y] = '#', ret += F;
      flow.add_edge(S, Conv(x, y), INF);
    }
    else{
      if(v[x][y] == '#') flow.add_edge(S, Conv(x, y), D);
      else flow.add_edge(Conv(x, y), T, F);
    }
    for(int d = 0; d < 4; d++){
      auto nx = x + dx[d], ny = y + dy[d];
      if(Bound(nx, ny)) continue;
      flow.add_edge(Conv(x, y), Conv(nx, ny), B);
    }
  }
  po(flow.maximum_flow(S, T) + ret);
};
