...
int v[111][111],cost[111][111];

void Main(){
    int n,p; ri(n, p);
	int s = 0,e = n + 1;
	MCMF mcmf(e + 1); mcmf.set_source(s); mcmf.set_sink(e);
	mcmf.add_edge(s, 1, p, 0); mcmf.add_edge(n, e, p, 0);
	for(int i = 1; i <= n; i++){
		for(int j = i + 1; j <= n; j++) ri(v[i][j]);
	}
	for(int i = 1; i <= n; i++){
		for(int j = i + 1; j <= n; j++) ri(cost[i][j]);
	}
	for(int i = 1; i <= n; i++) for(int j = i + 1; j <= n; j++) mcmf.add_edge(i, j, v[i][j], -cost[i][j]);
	for(int i = 1; i < n; i++) mcmf.add_edge(i, i + 1, INF, 0);
	auto [cost, fl] = mcmf.mcmf();
	po(-cost);
}
