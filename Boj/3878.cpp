...
void test_case(){
    ...
    if(n < 3){ // n < 3 and m < 3
        if(n == 1){
            ...
            cout << (line(w[0], w[1]).is_on_line(b[0]) ? "NO" : "YES") << '\n';
            return;
        }
        else{
            auto a = line(w[0], w[1]),y = line(b[0], b[1]);
            cout << (is_cross(a, y) ? "NO" : "YES") << '\n';
            return;
        }
    }
    polygon wp(w),bp(b);
    ...
    bp.convex_hull();
    for(auto& P : w){
        if(bp.point_in_polygon(P)){
            cout << "NO" << "\n";
            return;
        }
    }
    for(auto& P : b){
        if(wp.point_in_polygon(P)){
            cout << "NO" << "\n";
            return;
        }
    }
