...
void Main(){
	ri(N, M);
	int mx = 0;
	vti3 v(N); for(auto& [a, b, c] : v) ri(a, b, c), mx = max<int>({mx, a, b});
	vector<p> I(M); 
	vi X(mx + 1);
	for(const auto& [a, b, c] : v){
		for(int j = a; j <= b; j++) X[j] += c;
	}
	for(int i = 0; i < M; i++) ri(I[i][0], I[i][1], I[i][2], I[i][3]);
	int mn = INF;
	for(int i = 1; i < (1 << M); i++){
		int cost = 0;
		vi w(mx + 1);
		for(int j = 0; j < M; j++){
			if(i & (1 << j)){
				for(int k = I[j][0]; k <= I[j][1]; k++) w[k] += I[j][2];
				cost += I[j][3];
			}
		}
		bool flag = 0;
