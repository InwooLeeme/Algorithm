...
int F(char c, int d){
    if(c == '/'){
        if(d == 0 or d == 1) return (d == 0 ? 1 : 0);
        else return (d == 2 ? 3 : 2);
    }
    if(d == 1 or d == 2) return (d == 1 ? 2 : 1);
    else return (d == 0 ? 3 : 0);
}
...
void Main(){
    ...
    for(int d = 0; d < 4; d++){
        ...
        auto DFS = [&](int x, int y, int dep, int dir, auto&& DFS) -> void{
            vist[x][y][dir] = 1;
            mx = max<int>(mx, dep);
            if(dep != 1 and x == r and y == c and d == dir){
                flag = 1;
                return;
            }
            auto nx = x + kdx[dir], ny = y + kdy[dir];
            if(Bound(nx, ny) or board[nx][ny] == 'C') return;
            DFS(nx, ny, dep + 1, (board[nx][ny] == '.' ? dir : F(board[nx][ny], dir)), DFS);
        };
        DFS(r, c, 1, d, DFS);
        if(flag) cy.push_back(d);
        else w.push_back({d, mx});
    }
