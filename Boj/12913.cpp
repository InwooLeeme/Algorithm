...
void Main(){ 
	...
	for(int i = 0; i < n; i++){
		for(int j = 0; j < n; j++){
			char c; ri(c); double cc = (c - '0');
			if(i == j) continue;
			G[i].push_back({cc, j}); G[j].push_back({cc, i});
		}
	}
	using P = tuple<double, int, int>;
	min_pq<P> PQ; dist[0][0] = 0; PQ.push({dist[0][0], 0, 0});
	while(PQ.size()){
		auto [cdist, state, cur] = PQ.top(); PQ.pop();
		if(dist[cur][state] < cdist) continue;
		if(state > k) continue;
		for(const auto& [cost, nxt] : G[cur]){
			if(dist[nxt][state] > cdist + cost){
				dist[nxt][state] = cdist + cost;
				PQ.push({dist[nxt][state], state, nxt});
			}
			auto nstate = state + 1;
			if(nstate > k) continue;
			if(dist[nxt][nstate] > cdist + (cost / 2.0)){
				dist[nxt][nstate] = cdist + (cost / 2.0);
				PQ.push({dist[nxt][nstate], nstate, nxt});
			}
		}
	}
	double mn = INF;
	for(int i = 0; i <= k; i++){
		mn = min<double>(mn, dist[1][i]);
	}
	po(mn);
} 	
