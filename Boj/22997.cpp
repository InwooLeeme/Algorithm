...
void Main(){
	...
	vp w;
	int ret = 0;
	auto Update = [&](int sx, int sy, int ex, int ey, int v) -> void{
		FT.update(sx, sy, v);
		FT.update(sx, ey + 1, -v);
		FT.update(ex + 1, sy, -v);
		FT.update(ex + 1, ey + 1, v);
	};
	auto Query = [&](int x, int y) -> int{ return FT.get(x, y); };
	int n = 0;
	for(int i = 0; i < M + K; i++){
		int t; ri(t);
		if(t == 1){
			int a,b,r; ri(a, b, r);
			auto x = a + b,y = a - b + 500;
			Update(max<int>(1, x - r), max<int>(1, y - r), min<int>(1000, x + r), min<int>(1000, y + r), 1);
		}
		else if(t == 2){
			int a, b; ri(a, b);
			auto x = a + b, y = a - b + 500;
			n++;
			MX[n] = x, MY[n] = y, MV[n] = Query(x, y);
		}
		else if(t == 3){
			int idx; ri(idx);
			if(Query(MX[idx], MY[idx]) > MV[idx]) ret++;
			vist[idx] = 1;
		}
	}
	for(int i = 1; i <= n; i++) if(!vist[i] and Query(MX[i], MY[i]) > MV[i]) ret++;
	po(ret);
}	
