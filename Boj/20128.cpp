...
void solve(){
	...
	minpq<pii> pq; pq.push({dist[1][0] = 0, 1});
	while(si(pq)){
		auto [cdist, cur] = pq.top(); pq.pop();
		if(cdist != dist[cur][cdist & 1]) continue;
		for(const auto& [cost, nxt] : g[cur]){
			auto ncost = cdist + cost;
			if(dist[nxt][ncost & 1] > ncost){
				pq.push({dist[nxt][ncost & 1] = ncost, nxt});
			}
		}
	}
	for(int i = 1; i <= n; i++){
		for(int j = 1; j >= 0; j--) cout << (dist[i][j] == INF ? -1 : dist[i][j]) << " \n"[j == 0];  
	}
}
