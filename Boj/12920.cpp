...
void Main(){
	int n,m; ri(n, m);
	for(int i = 0; i < n; i++) ri(A[i], B[i], C[i]);
	vector<pii> v;
	for(int i = 0; i < n; i++){
		int sum = 0;
		for(int j = 0; j <= 14; j++){
			if(sum + (1 << j) <= C[i]){
				v.push_back({A[i] * (1 << j), B[i] * (1 << j)});
				sum += (1 << j);
			}
		}
		if(sum < C[i]) v.push_back({A[i] * (C[i] - sum), B[i] * (C[i] - sum)});
	}
	// knap sack
	for(int i = 0; i < v.size(); i++){
		for(int j = m; j >= v[i].first; j--) D[j] = max<int>(D[j], D[j - v[i].first] + v[i].second);
	}
	po(D[m]);
}
