...
void Main(){
    ...
    vi P(n * m + 1);
    iota(all(P), 0);
    auto Find = [&](int x, auto&& Find) -> int{ return P[x] == x ? x : P[x] = Find(P[x], Find); };
    auto Union = [&](int a, int b) -> bool{
        a = Find(a, Find),b = Find(b, Find);
        if(a == b) return 0; P[b] = a; return 1;
    };
    auto C = [&](int x, int y) -> int{ return x * m + y; };
    for(int i = 0; i < n; i++){
        for(int j = 0; j < m; j++){
            auto d = string("WSEN").find(v[i][j]);
    ...
    int ret = 0;
    for(int i = 0; i < n * m; i++) ret += (i == Find(i, Find));
    po(ret);
}
