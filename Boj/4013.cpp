...
void Main(){
	int n,m; ri(n, m); SCC G(n); while(m--){
		int a,b; ri(a, b);
		G.AddEdge(a, b);
	}
	G.GetSCC();
	vi v(n + 1); for(int i = 1; i <= n; i++) ri(v[i]);
	int s,p; ri(s, p); vector<bool> sccRest(G.scc_cnt + 1); 
	for(int i = 0; i < p; i++){
		int t; ri(t);  sccRest[G.scc[t]] = 1;
	}
	vector<int> D(n + 1),sccv(G.scc_cnt + 1);
	for(int i = 1; i <= n; i++) sccv[G.scc[i]] += v[i];
	// Build DAG
	auto DAG = G.GetDAG(); 
	vi indegree(G.scc_cnt + 1);
	queue<int> Q;
	int st = G.scc[s]; 
	D[st] = sccv[st];
	for(auto& i : DAG) for(auto& j : i) indegree[j]++;
	for(int i = 1; i <= G.scc_cnt; i++) if(!indegree[i]){
		Q.push(i);
	}
	bool flag = 0;
	// topo sort
	while(Q.size()){
		...
		for(auto nxt : DAG[cur]){
			if(flag) D[nxt] = max(D[nxt], D[cur] + sccv[nxt]);
			if(--indegree[nxt] == 0) Q.push(nxt);
		}
	}
	int mx = 0;
	for(int i = 1; i <= G.scc_cnt; i++) if(sccRest[i]) mx = max<int>(mx, D[i]);
	po(mx);
}
