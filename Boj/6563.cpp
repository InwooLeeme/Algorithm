...
void C(){
	for(int i = 0; i < 27; i++){
		for(int j = 0; j < 222; j++) for(int k = 0; k < 222; k++) dist[i][j][k] = INF;
	}
}
...
void Main(){
	...
	while(cin >> n and n){
		...
		while(cin >> a >> b and a and b){
			string s; cin >> s;
			for(auto& c : s) dist[c - 'a'][a][b] = min<int>(dist[c - 'a'][a][b], 1);
		}
		// floyd
		for(int r = 0; r < 26; r++){
			for(int k = 1; k <= n; k++) for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++){
				dist[r][i][j] = min<int>(dist[r][i][j], dist[r][i][k] + dist[r][k][j]);
			}
		}
		int x,y;
