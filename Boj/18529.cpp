...
vector<pii> C[2505];
vector<pii> G[2505];
int Conv(int x, int y){ return (n * x + y); }
vi dist(2505, INF);
...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < n; j++){
		ri(v[i][j]);
		C[v[i][j]].push_back({i, j});
	}
	for(int i = 2; i <= k; i++){
		for(const auto& [a, b] : C[i - 1]){
			for(const auto& [c, d] : C[i]){
				auto D = abs(a - c) + abs(b - d);
				G[Conv(a, b)].push_back({D, Conv(c, d)});
			}
		}
	}
	min_pq<pii> PQ;
	for(const auto& [a, b] : C[1]){
		dist[Conv(a, b)] = 0;
		PQ.push({dist[Conv(a, b)] = 0, Conv(a, b)});
	}
	while(PQ.size()){
	...
	int mn = INF;
	for(const auto& [a, b] : C[k]) mn = min<int>(mn, dist[Conv(a, b)]);
	po(mn == INF ? -1 : mn);
}
