...
void Main(){
	int n,m;
	while(cin >> n >> m){
		vvi D(n + 1, vi(10101, INF));
		vector<vti3> G(n + 1);
		while(m--){
			int a,b,c,d; ri(a, b, c, d); // cost, money
			G[a].push_back({c, d, b});
			G[b].push_back({c, d, a});
		}
		min_pq<ti3> PQ;
		int s,e,k; ri(s, e, k);
		D[s][0] = 0;
		PQ.push({0, D[s][0], s}); // money, cost, node
		while(PQ.size()){
			auto [M, cdist, cur] = PQ.top(); PQ.pop();
			if(D[cur][M] < cdist) continue;
			for(const auto& [cost, nxtM, nxt] : G[cur]){
				auto nxtMoney = M + nxtM;
				if(nxtMoney > k) continue;
				auto ncost = cost + cdist;
				if(D[nxt][nxtMoney] > ncost){
					D[nxt][nxtMoney] = ncost;
					PQ.push({nxtMoney, D[nxt][nxtMoney], nxt});
				}
			}
		}
		int mn = INF;
		for(int i = 0; i <= k; i++) mn = min<int>(mn, D[e][i]);
		po(mn);
	}
}	
