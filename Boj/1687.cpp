...
void Main(){
    ...
    for(int i = 1; i <= n; i++) for(int j = 1; j <= m; j++){
        char c; ri(c);
        C[i][j] = (c == '0' ? 1 : 0) + C[i - 1][j] + C[i][j - 1] - C[i - 1][j - 1];
    }
    for(int r1 = 1; r1 <= n; r1++) for(int c1 = 1; c1 <= m; c1++){
        int r2 = r1, c2 = c1;
        while(c2 <= m and C[r2][c2] - C[r1 - 1][c2] - C[r2][c1 - 1] + C[r1 - 1][c1 - 1] == (r2 - r1 + 1) * (c2 - c1 + 1)) c2++;
        c2--;
        while(r2 <= n){
            while(C[r2][c2] - C[r1 - 1][c2] - C[r2][c1 - 1] + C[r1 - 1][c1 - 1] != (r2 - r1 + 1) * (c2 - c1 + 1)) c2--;
            mx = max<int>(mx, (r2 - r1 + 1) * (c2 - c1 + 1));
            r2++;
        }
    }
    po(mx);
