...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
		ri(I[i][j]);
		if(I[i][j]) {S.insert({i, j});}
	}
	auto Check = [&](const int mid) -> bool{ // 최대 차이가 mid 이하로만 이동해서 모든 포인트에 도달할 수 있는가?
		...
		while(Q.size()){
			...
			for(int d = 0; d < 4; d++){
				...
				if(Bound(nx, ny) or abs(v[nx][ny] - v[x][y]) >= mid or dist[nx][ny] != -1) continue;
				dist[nx][ny] = dist[x][y] + 1;
				Q.push({nx, ny});
			}
		}
		for(const auto& [a, b] : S){
			if(dist[a][b] == -1) return 0;
		}
		return 1;
	};
	int lo = -1, hi = int(1e10);
	while(lo + 1 < hi){
		int m = lo + hi >> 1;
		if(!Check(m)) lo = m;
		else hi = m;
