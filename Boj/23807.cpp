...
int32_t main(){
    ...
    int st,en,p; cin >> st >> en >> p;
    vector<int> P(p);
    for(auto& i : P) cin >> i;
    auto dist = Dijkstra(st);
    for(auto& i : P) M[i] = Dijkstra(i);
    int mn = INF;
    for(int i = 0; i < p; i++){
        for(int j = 0; j < p; j++){
            for(int k = 0; k < p; k++){
                if((j == k) or (i == k) or (i == j)) continue;
                int r = 0;
                if(dist[P[i]] == INF) continue;
                r += dist[P[i]];
                if(M[P[i]][P[j]] == INF) continue;
                r += M[P[i]][P[j]];
                if(M[P[j]][P[k]] == INF) continue;
                r += M[P[j]][P[k]];
                if(M[P[k]][en] == INF) continue;
                r += M[P[k]][en];
                mn = min<int>(mn, r);
            }
        }
