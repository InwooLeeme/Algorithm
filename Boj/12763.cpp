...
int n,m,t,k;
vti3 G[111];
vvi dist(111, vi(11111, INF)); // D[i][j] : i번정점에 j원에 왔을 때 걸리는 최단시간
...
void Main(){
	...
	while(PQ.size()){
		auto [cdist, cmoney, cur] = PQ.top(); PQ.pop();
		if(dist[cur][cmoney] < cdist) continue;
		for(const auto& [cost, money, nxt] : G[cur]){
			auto nmoney = cmoney + money;
			if(nmoney > t) continue;
			if(dist[nxt][nmoney] > cdist + cost){
				dist[nxt][nmoney] = cdist + cost;
				PQ.push({dist[nxt][nmoney], nmoney, nxt});
			}
		}
	}
    for(int i = 0; i <= t; i++){
