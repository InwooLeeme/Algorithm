...
void Main(){
    ...
    auto Check = [&](const int L, const int R) -> bool{
        ...
        for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++){
            if(L <= v[i][j] and v[i][j] <= R) scc.AddEdge(i, j);
        }
        scc.GetSCC();
        for(int i = 1; i <= n; i++) if(scc.scc[1] != scc.scc[i]) return 0;
        return 1;
    };
    int mn = INF;
    for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++){
        int ok = v[i][j] - 1, not_ok = 150'002;
        while(ok + 1 < not_ok){
            auto mid = ok + not_ok >> 1;
            if(!Check(v[i][j], mid)) ok = mid;
            else not_ok = mid;
        }
        if(not_ok != 150'002) mn = min<int>(mn, not_ok - v[i][j]);
    }
    po(mn);
}
