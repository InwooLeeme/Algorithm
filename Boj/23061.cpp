...
void Main(){
	...
	int sz = 1'000'010;
	vti3 u;
	vector<int> w(n),v(n); for(int i = 0; i < n; i++) ri(w[i], v[i]);
	vi D(sz);
	for(int i = 0; i < n; i++){
		for(int j = sz - 1; j >= w[i]; j--) D[j] = max<int>(D[j], D[j - w[i]] + v[i]);
	}
	for(int i = 1; i <= m; i++){
		int t; ri(t);
		u.push_back({D[t], t, i});
	}
	sort(u.begin(), u.end(), [&](ti3& x, ti3& y){
		auto& [a, b, idx1] = x;
		auto& [c, d, idx2] = y;
		return b*c == a*d ? idx1 < idx2 : b*c < a*d;
	});
	auto& [a, b, ret] = u[0];
	po(ret);
}
