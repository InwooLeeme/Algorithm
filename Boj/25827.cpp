...
int Split(string s){
    ...
    stringstream in(s);
    string temp;
    while(in >> temp) ret.pb(temp);
    return stoi(ret[0])*3600 + stoi(ret[1]) * 60 + stoi(ret[2]);
}
...
void Main(){
    ...
    SegTree<1 << 17> ST;
    for(int i = 0; i < N; i++){
        int t; string a,b; ri(t, a, b);
        int S = Split(a), E = Split(b);
        if(t == 1) ST.Update(S + 1, E, 1);
        else po(ST.Query(S + 1, E));
    }
}   
