...
int N,F,D;
int in(int x){ return x + 101;}
int out(int x){ return x + 201;}
int Drink(int x){ return x + 301;}

void Main(){
	ri(N, F, D);
	Dinic<int> flow(450);
	int S = 0,E = 448;
	for(int i = 1; i <= F; i++) flow.add_edge(S, i, 1);	
	for(int i = 1; i <= N; i++) flow.add_edge(in(i), out(i), 1);
	for(int i = 1; i <= D; i++) flow.add_edge(Drink(i), E, 1);	
	for(int i = 1; i <= N; i++){
		int a,b; ri(a, b);
		for(int j = 0; j < a; j++){
			int x; ri(x);
			flow.add_edge(x, in(i), 1);
		}
		for(int j = 0; j < b; j++){
			int x; ri(x);
			flow.add_edge(out(i), Drink(x), 1);
		}
	}
