...
void Main(){
  ri(n, m, q);
  vvvi dist(n + 1, vvi(n + 1, vi(n + 1, INF)));
  for(int i = 0; i < m; i++){
    int a,b,c; ri(a, b, c); 
    for(int j = 1; j <= n; j++) dist[j][a][b] = min<int>(dist[j][a][b], c);
  }
  for(int l = 1; l <= n; l++) for(int k = 1; k <= n; k++) for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++){
    if(l == k) continue;
    dist[l][i][j] = min<int>(dist[l][i][j], dist[l][i][k] + dist[l][k][j]);
  }
  while(q--){
    int s,k,e; ri(s, k, e);
    if(dist[k][s][e] == INF) po("No");
    else po(dist[k][s][e]);
  }
}
