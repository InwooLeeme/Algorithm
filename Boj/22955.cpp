	...
	while(PQ.size()){
		...
		if(v[x][y] != 'X'){
			for(int d = 0; d < 4; d++){
				...
				auto cost = ((d == 0 or d == 2) ? 1 : 5);
				if(Bound(nx, ny) or v[nx][ny] == 'D') continue;
				if(v[nx][ny] != 'L' and d == 1) continue;
				if(v[x][y] != 'L' and d == 3) continue;
				if(dist[nx][ny] > cost + cdist){
					PQ.push({dist[nx][ny] = cost + cdist, nx, ny});
				}
			}
		}
		else{
			auto nx = x,ny = y;
			while(1){ 
				if(v[nx][ny] != 'X' or Bound(nx, ny)) break;
				nx += 1; 
			}
			if(v[nx][ny] == 'D') continue;
			if(dist[nx][ny] > 10 + cdist){
				PQ.push({dist[nx][ny] = cdist + 10, nx, ny});
			}
		}
	}
	po("dodo sad");
