...
void Main(){
	...
	queue<vvi> Q;
	vvi s(2, vi(4));
	for(int i = 0,x = 1; i < 4; i++,x++) s[0][i] = x;
	for(int i = 3,x = 5; i >= 0; i--,x++) s[1][i] = x;
	Q.push(s); dist[s] = 0;
	while(Q.size()){
		...
		for(int d = 0; d < 4; d++){
			if(d == 0){
				auto nxt = cur;
				for(int i = 0; i < 4; i++) swap(nxt[0][i], nxt[1][i]);
				if(dist.count(nxt)) continue;
				dist[nxt] = dist[cur] + 1; Q.push(nxt);
			}
			else if(d == 1){
				auto nxt = cur;
				rotate(nxt[0].begin(), nxt[0].end() - 1, nxt[0].end());
				rotate(nxt[1].begin(), nxt[1].end() - 1, nxt[1].end());
				if(dist.count(nxt)) continue;
				dist[nxt] = dist[cur] + 1; Q.push(nxt);
			}
			else if(d == 2){
				auto nxt = cur;
				nxt[0][1] = cur[0][2]; nxt[1][1] = cur[0][1]; nxt[1][2] = cur[1][1]; nxt[0][2] = cur[1][2];
				if(dist.count(nxt)) continue;
				dist[nxt] = dist[cur] + 1; Q.push(nxt);
			}
			else if(d == 3){
				auto nxt = cur;
				swap(nxt[0][0], nxt[1][3]);
				if(dist.count(nxt)) continue;
				dist[nxt] = dist[cur] + 1; Q.push(nxt);
			}
		}
