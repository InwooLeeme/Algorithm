...
void Main(){
	...
	sort(all(v), [&](pt& a, pt& b){
		return min<int>(a[0], a[2]) < min<int>(b[0], b[2]);
	});
	int e = max<int>(v[0][0], v[0][2]), cur = v[0][4];
	for(int i = 1; i < n; i++){
		auto [a, b, c, d, idx] = v[i];
		auto l = min<int>(a, c);
		if(l <= e) UF.Union(cur, idx);
		e = max<int>({e, a, c});
		cur = idx;
	}
	sort(all(v), [&](pt& a, pt& b){
		return min<int>(a[1], a[3]) < min<int>(b[1], b[3]);
	});
	e = max<int>(v[0][1], v[0][3]), cur = v[0][4];
	for(int i = 1; i < n; i++){
		auto [a, b, c, d, idx] = v[i];
		auto l = min<int>(b, d);
		if(l <= e) UF.Union(cur, idx);
