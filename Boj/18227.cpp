...
struct HLD {
	int n, r;
	int sz[1 << 18], dep[1 << 18], par[1 << 18];
	int top[1 << 18], in[1 << 18], out[1 << 18];
	vector<int> adj[1 << 18], inp[1 << 18];
	bitset<1 << 18> visited;
	void DFS(int cur = 1) {
		visited[cur] = 1;
    ...
    void Update(int i){
        while(top[i] != r){
            int st = top[i];
            ST.Update(in[st], in[i], 1);
            i = par[st];
        }
        ST.Update(in[r], in[i], 1);
    }

	int Query(int i) {
		return ST.Query(in[i], in[i]) * (dep[i] + 1);
	}
}HLD;
