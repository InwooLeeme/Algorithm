...
int board[55][55];
int dist[55][55][1 << 5];
int toggle(int state, int x){ return (state | (1 << x));}
bool Have(int state, int x){ return (state & (1 << x)); }
bool Bound(int x, int y){ return x < 0 or x >= n or y < 0 or y >= n; }
const int ax[8] = {1, 2, 2, 1, -1, -2, -2, -1}, ay[8] = {-2, -1, 1, 2, 2, 1, -1, -2};
...
void Main(){	
	...
	min_pq<I> PQ;
	dist[l][r][0] = 0; PQ.push({dist[l][r][0] = 0, 0, l, r});
	while(PQ.size()){
		auto [cdist, state, x, y] = PQ.top(); PQ.pop();
		...
		for(int d = 0; d < 8; d++){
			auto nx = x + ax[d], ny = y + ay[d];
			if(Bound(nx, ny)) continue;
			auto nstate = (board[nx][ny] and !Have(state, board[nx][ny]) ? toggle(state, board[nx][ny]) : state);
			if(dist[nx][ny][nstate] > cdist + a){
				dist[nx][ny][nstate] = cdist + a;
				PQ.push({dist[nx][ny][nstate], nstate, nx, ny});
			}
		}
		for(int d = 0; d < 4; d++){
			auto nx = x, ny = y;
			while(1){
				if(Bound(nx + bx[d], ny + by[d])) break;
				nx += bx[d], ny += by[d];
				auto nstate = (board[nx][ny] and !Have(state, board[nx][ny]) ? toggle(state, board[nx][ny]) : state);
				if(dist[nx][ny][nstate] > cdist + b){
					dist[nx][ny][nstate] = cdist + b;
					PQ.push({dist[nx][ny][nstate], nstate, nx, ny});
				}
			}
		}
		for(int d = 0; d < 4; d++){
			auto nx = x, ny = y;
			while(1){
				if(Bound(nx + dx[d], ny + dy[d])) break;
				nx += dx[d], ny += dy[d];
				auto nstate = (board[nx][ny] and !Have(state, board[nx][ny]) ? toggle(state, board[nx][ny]) : state);
				if(dist[nx][ny][nstate] > cdist + c){
					dist[nx][ny][nstate] = cdist + c;
					PQ.push({dist[nx][ny][nstate], nstate, nx, ny});
				}
			}
		}
