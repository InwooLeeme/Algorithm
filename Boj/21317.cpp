...
void Main(){ 
    ...
    ri(k);
    for(int i = 2; i <= n; i++) D[i][0] = D[i][1] = INF;
    for(int i = 1; i < n; i++){
        for(int j = 0; j < 2; j++){
            D[i + 1][j] = min<int>(D[i + 1][j], D[i][j] + x[i]);
            D[i + 2][j] = min<int>(D[i + 2][j], D[i][j] + y[i]);
            if(!j) D[i + 3][1] = min<int>(D[i + 3][1], D[i][j] + k);
        }
    }
    po(min<int>(D[n][1], D[n][0]));
} 		
