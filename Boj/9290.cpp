...
void Sol(){
	vs v(3); ri(v);
	char c; ri(c);
	auto Bound = [&](int x, int y) -> bool{ return x < 0 or x >= 3 or y < 0 or y >= 3; };
	for(int i = 0; i < 3; i++){
		for(int j = 0; j < 3; j++){
			for(int d = 0; d < 8; d++){
				int cnt = 0;
				auto nx = i, ny = j;
				while(1){
					if(Bound(nx, ny) or v[nx][ny] != c) break;
					nx += ddx[d], ny += ddy[d];
					cnt++;
				}
				if(cnt == 3){
	...
	vp w;
	for(int i = 0; i < 3; i++) for(int j = 0; j < 3; j++) if(v[i][j] == '-') w.push_back({i, j});
	vi chk;
	for(int i = 0; i < w.size(); i++) chk.push_back(i < 1 ? 0 : 1);
	do{
		vs cur(3);
		for(int i = 0; i < 3; i++) cur[i] = v[i];
			...
			for(int j = 0; j < 3; j++){
				if(cur[i][j] == c){
					for(int d = 0; d < 8; d++){
						...
						auto nx = i, ny = j;
						while(1){
							if(Bound(nx, ny) or cur[nx][ny] != c) break;
							nx += ddx[d], ny += ddy[d];
							cnt++;
						}
						if(cnt == 3){
							for(const auto& i : cur) cout << i << "\n";
							return;
						}
					}
				}
