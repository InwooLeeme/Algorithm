...
void Sol(string& s){
	...
	auto DFS = [&](int dep, int x, int y, int dir, auto&& DFS) -> void{
		if(dep == 2){
			if(board[x][y] == 'X') xb++;
			else if(board[x][y] == 'O') ob++;
		}
		auto nx = x + dx[dir], ny = y + dy[dir];
		if(!Bound(nx, ny) and board[nx][ny] == board[x][y]) DFS(dep + 1, nx, ny, dir, DFS);
	};
	if(o > x or (x - o) >= 2) {No(); return;}
	if(x < 3 and o < 3) {No(); return;}
	...
	else if(xb and ob) {No(); return;}
	else if((!xb and ob) and x == o) {Yes(); return;}
	else if((!xb and ob) and x > o) {No(); return;}
	else if(!xb and !ob and (x + o) < 9) {No(); return;}
	else if(!xb and !ob and (x + o) == 9) {Yes(); return;}
}
