...
void BFS(int idx, int r, int c){
	...
	while(Q.size()){
		...
		for(int d = 0; d < 4; d++){
			...
			if(v[nx][ny] == 'X' or dist[idx][nx][ny] != -1) continue;
...
int Sol(vi& w){
	int ret = dist[22][cod[w[0]].fi][cod[w[0]].se];
	if(ret == -1) return INF;
	for(int i = 1; i < (int)w.size(); i++){
		auto prv = w[i - 1];
		auto r = cod[w[i]].fi,c = cod[w[i]].se;
		auto x = dist[prv][r][c];
		if(x == -1) return INF;
		ret += x;
	}
...
void Main(){
	...
	BFS(22, cod[22].fi, cod[22].se);
	for(int i = 0; i < K; i++) BFS(i, cod[i].fi, cod[i].se);
	vi w;
	for(int i = 0; i < K; i++) w.push_back(i < 5 ? 0 : 1);
	int mn = INF;
	do{
		vi cur;
		for(int i = 0; i < K; i++) if(!w[i]) cur.push_back(i);
		do{
			mn = min<int>(mn, Sol(cur));
