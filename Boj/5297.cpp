...
void Main(){ 
	...
	vi dist(n + 1, INF); min_pq<pii> PQ;
	PQ.push({dist[s] = 0, s});
	while(PQ.size()){
		auto [cdist, cur] = PQ.top(); PQ.pop();
		if(dist[cur] < cdist) continue;
		if(cur == e){
			cout << cdist << " native(s) on the easiest way for Captain Jack." << "\n";
			return;
		}
	...
	po("It's over with Captain Jack. At least till Pirates of the Caribbean 3.");
} 
