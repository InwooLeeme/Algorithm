...
int32_t main(){
    ...
    set<int> already;
    for(int i = 0; i < n; i++){
        int idx, x, y, z, w; sf5(idx, x, y, z, w);
        a.push_back({idx, x});
        b.push_back({idx, y});
        c.push_back({idx, z});
        d.push_back({idx, w});
    ...
    pf1(a[0].first); already.insert(a[0].first);
    for(auto& [idx, score] : b){
        if(already.count(idx)) continue;
        already.insert(idx);
        pf1(idx);
        break;
    }
    for(auto& [idx, score] : c){
        if(already.count(idx)) continue;
        already.insert(idx);
        pf1(idx);
        break;
    }
    for(auto& [idx, score] : d){
        if(already.count(idx)) continue;
        already.insert(idx);
        pf1(idx);
        break;
