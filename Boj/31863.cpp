...
void f(int dep, int x, int y, int d){
	...
	if(v[x][y] == '*'){
		...
		Cnt[x][y]++;
		if(Cnt[x][y] >= 2) Q.push({x, y});
	}
	auto nx = x + dx[d], ny = y + dy[d];
	if(Bound(nx, ny) or v[nx][ny] == '|') return;
	f(dep + 1, nx, ny, d);
}
...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
		...
		else if(v[i][j] == '*' or v[i][j] == '#') nret++;
	}
	while(!Q.empty()){
		auto [x, y] = Q.front(); Q.pop(); debug(x, y, v[x][y]);
