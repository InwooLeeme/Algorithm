...
int Conv(int x, int y){ return x*50 + y; }
...
void Main(){
	...
	int m; ri(m);
	while(m--){
		int a, b, c, d; ri(a, b, c, d); --a,--c;
		auto s = Conv(a, b), e = Conv(c, d);
		G[s].push_back({c, e});
		G[e].push_back({a, s});
	}	
	auto Dijkstra = [&](int sn, int s, int e, int T){
		vi dist(666, INF);
		min_pq<ti3> PQ;
		dist[s] = 0; PQ.push({dist[s], sn, s});
		while(PQ.size()){
			auto [cdist, curState, cur] = PQ.top(); PQ.pop();
			if(cur == e) break;
			if(dist[cur] < cdist) continue;
			for(const auto& [nstate, nxt] : G[cur]){
				auto cost = cdist + (nstate != curState ? T : 1);
				if(dist[nxt] > cost){
					dist[nxt] = cost;
					PQ.push({dist[nxt], nstate, nxt});
				}
			}
		}
		return dist;
	...
	int Q; ri(Q);
	while(Q--){
		int t,a,b,c,d; ri(t, a, b, c, d); --a,--c;
		auto s = Conv(a, b), e = Conv(c, d);
		auto dist = Dijkstra(a, s, e, t);
		po(dist[e]);
	}
}	
