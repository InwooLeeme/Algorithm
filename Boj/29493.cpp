	...
	for(int i = 0; i <= 100; i++){
		for(int j = 0; j <= 100; j++){
			for(int d = 0; d < 4; d++){
				...
				g[{i, j}].pb({1, nx, ny}); g[{nx, ny}].pb({1, i, j});
			}
		}
	}
	for(int i = 0; i < n; i++){
		int x1,x2,y1,y2; ri(x1, y1, x2, y2);
		g[{x1, y1}].pb({0, x2, y2});
		g[{x2, y2}].pb({0, x1, y1});
	}
	ri(ex, ey);
	minpq<tuple<int, int, int, int>> pq;
	vector<vp> dist(111, vp(111, {INF, INF})); dist[0][0] = {0, 0}; pq.push({0, 0, 0, 0});
	while(si(pq)){
		auto [cdist, cadd, x, y] = pq.top(); pq.pop();
		if(make_pair(cdist, cadd) != dist[x][y]) continue;
		if(x == ex and y == ey){
			po(cadd);
			return;
		}
		for(const auto& [cost, x2, y2] : g[{x, y}]){
			int ndist = cdist + 1; 
            int nadd = cadd + cost;
            if (make_pair(ndist, nadd) < dist[x2][y2]) {
                dist[x2][y2] = {ndist, nadd};
                pq.push({ndist, nadd, x2, y2});
            }
		}
	}
}
