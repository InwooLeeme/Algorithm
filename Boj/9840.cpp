...
void solve(){
	...
	for(int x = 0; x < N; x++){
		auto [r, c] = R[x];
		auto dist = bfs(r, c);
		for(const auto& [y, d] : R){
			auto cur = dist[d.fi][d.se];
			if(cur == -1) cur = INF;
			cost[x][y] = cur;
		}
	}
    ...
    for(int mask = 0; mask <= FULL; mask++){
        for(int i = 0; i <= K; i++){
            ...
            for(int f = 1; f <= K; f++){
                ...
                if(cost[i][f] >= INF) continue;
                dp[mask | bit][f] = min<int>(dp[mask | bit][f], cur + cost[i][f]);
            }
        }
    }
	int res = INF;
	for(int i = 0; i <= K; i++){
        if(dp[FULL][i] >= INF or cost[i][K + 1] >= INF) continue; 
        res = min<int>(res, dp[FULL][i] + cost[i][K + 1]);
    }
	po(res == INF ? -1 : res);
}
