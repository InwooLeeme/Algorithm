...
void Main(){
	int n; ri(n);
	queue<ti3> Q; Q.push({0, 1, 0}); dist[1][0] = 0;
	while(Q.size()){
		auto [time, cnt, clip] = Q.front(); Q.pop();
		for(auto [ncnt, nclip] : vp{{cnt, cnt}, {cnt + clip, clip}, {cnt - 1, clip}}){
			if(0 <= ncnt and ncnt <= 2000 and 0 <= nclip and nclip <= 1000 and !dist[ncnt][nclip]){
				dist[ncnt][nclip] = 1;
				Q.push({time + 1, ncnt, nclip});
				if(ncnt == n){
					po(time + 1);
					return;
				}
