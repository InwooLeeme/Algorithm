...
void Main(){
	...
	M[{0, 0}] = 0;
	for(int i = 0; i < n; i++){
		int a,b; string s; ri(a, b, s);
		if(!M.count(s)) M[s] = idx++;
		v.push_back({a, b});
		I.push_back(M[s]);
	}
	vvi w(n + 1, vi(n + 1, INF));
	for(int i = 0; i < (int)v.size(); i++){
		auto [a, b] = v[i];
		for(int j = 0; j < (int)v.size(); j++){
			auto [c, d] = v[j];
			if(i == j) continue;
			auto cdist = abs(a - c) + abs(b - d);
			w[i][j] = cdist;
		}
	}
	int N = M.size();
	...
	auto TSP = [&](int cur, int vist, auto&& TSP) -> int{
		if(vist == (1 << N) - 1) return w[cur][0];
		int& ret = D[cur][vist];
		if(ret != 0) return ret;
		ret = INF;
		for(int i = 0; i < (int)v.size(); i++){
			if(vist & (1 << I[i])) continue;
			if(w[cur][i] == INF) continue;
			ret = min<int>(ret, TSP(i, vist | (1 << I[i]), TSP) + w[cur][i]);
		}
		return ret;
	};
	po(TSP(0, 1, TSP));
