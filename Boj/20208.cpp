...
void f(int x, int y, int curH = m){
	if(abs(st.first - x) + abs(st.second - y) <= curH){
		chmax(mx, (int)v.size());
	}
	for(int i = 0; i < w.size(); i++){
		auto D = abs(x - w[i].first) + abs(y - w[i].second);
		if(vist[i] or curH < D) continue;
		vist[i] = 1;
		v.push_back(i);
		f(w[i].first, w[i].second, curH - D + H);
		v.pop_back();
		vist[i] = 0;
	}
}
...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < n; j++){
		int t; ri(t);
		if(t == 1) st = {i, j};
		if(t == 2) w.push_back({i, j});
	}
	f(st.first, st.second, m);
	po(mx);
}	
