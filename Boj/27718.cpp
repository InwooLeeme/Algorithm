...
struct point {
	...
	friend int is_cross(point a, point b, point c, point d) { // cross : -1(X), intersect : 1(X or L)
		...
		if (ab == 0 && cd == 0) {
			if (a > b) swap(a, b);
			if (c > d) swap(c, d);
			if (!ccw(a, b, c) and !ccw(a, b, d) and !ccw(c, d, a) and !ccw(c, d, b) and c < b and a < d) return 3; // intersect INF
			return c <= b && a <= d;
		}
		return ab <= 0 && cd <= 0;
	}
...
void Main(){
	int n; ri(n);
	vector<line> v(n); ri(v);
	vvi ret(n, vi(n));
	for(int i = 0; i < n; i++){
		for(int j = 0; j < n; j++){
			auto res = is_cross(v[i], v[j]);
			if(res == -1) ret[i][j] = 2;
			else if(res == 1) ret[i][j] = 1;
			else ret[i][j] = res;
		}
	}
	for(auto& i : ret){
