...
void Sol(){
    ...
    for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
        if(board[i][j] == 'o') { dist[i][j][0] = 0; Q.push({0, 0, i, j}); }
        else if(board[i][j] == '*'){
            bit |= (1 << dustcnt); board[i][j] = char(dustcnt + '0'); dustcnt++;
        }
    }
    while(Q.size()){
        ...
        for(int d = 0; d < 4; d++){
            ...
            if(Bound(nx, ny) or board[nx][ny] == 'x' or dist[nx][ny][state] != -1) continue;
            if(isdigit(board[nx][ny])){
                auto nstate = (!Have(state, board[nx][ny] - '0') ? toggle(state, board[nx][ny] - '0') : state);
                auto ncnt = (!Have(state, board[nx][ny] - '0') ? cnt + 1 : cnt);
