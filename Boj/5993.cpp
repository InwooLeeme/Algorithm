...
vector<vector<char>> Rotate(vector<vector<char>>& v){
	vector<vector<char>> ret(M, vector<char>(N, '.')), temp(M, vector<char>(N, '.'));
	for(int i = 0; i < M; i++) for(int j = 0; j < N; j++){
		temp[i][j] = (char)v[N - 1 - j][i];
	}
	for(int i = 0; i < M; i++) for(int j = 0; j < N; j++) ret[i][j] = temp[i][j];
	return ret;
}
...
void Main(){
	...
	while(Q.size()){
		...
		for(int d = 0; d < 8; d++){
			...
			if(nx < 0 or nx >= (int)board.size() or y < 0 or y >= (int)board[0].size()) continue;
			if(dist[nx][ny] != -1 or board[nx][ny] == '*') continue;
			dist[nx][ny] = dist[x][y] + 1; Q.push({nx, ny});
		}
	}
	for(int i = 0; i < board.size(); i++){
		for(int j = 0; j < board[0].size(); j++) mx = max<int>(mx, dist[i][j]);
	}
	po(mx);
}	
