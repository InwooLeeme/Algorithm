...
void Main(){
	ri(n, m);
	for(int i = 0; i < 3 * n; i++) ri(v[i]);
	for(int x = 0; x < n; x++) for(int y = 0; y < m; y++){
		string cur = "";
		int px = -1,py = -1,mnx = INF, mxx = -INF, mny = INF, mxy = -INF;
		...
		string temp;
		for(auto& i : cur) if(i == '+' or i == '=') i = ' ';
		stringstream in(cur);
		vs w;
		while(in >> temp) w.push_back(temp);
		bool f = (stoi(w[0]) + stoi(w[1]) == stoi(w.back()));
		if(f){
			v[mnx][mny - 1] = '*';
			v[mnx][mxy + 1] = '*';
			...
			v[px][py] = '/'; v[px + 1][py - 1] = '/'; v[px - 1][py + 1] = '/';
		}
	}
	for(int i = 0; i < 3 * n; i++) po(v[i]);
} 		
