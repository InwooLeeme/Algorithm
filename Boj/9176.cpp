...
ll DB[9] = {2047, 8388607, 536870911, 137438953471, 2199023255551, 8796093022207, 140737488355327, 9007199254740991, 576460752303423487};
int EXP[9] = {11, 23, 29, 37, 41, 43, 47, 53, 59};
vi Fact[9] = {{23, 89}, {47,178481}, {233,1103,2089}, {223,616318177}, {13367,164511353}, {431,9719,2099863}, {2351,4513,13264529}, {6361,69431,20394401}, {179951,3203431780337}};
...
void Main(){
	...
	for(int i = 0; i < 9; i++){
		...
		for(int j = 0; j < (int)Fact[i].size(); j++){
			cout << Fact[i][j] << (j == (int)Fact[i].size() - 1 ? " = " : " * ");
		}
		cout << DB[i] << " = ";
		cout << "( " << "2 ^ " << EXP[i] << " ) - 1\n";
	}
