...
void Main(){
	int n,m; ri(n, m); int lim = n * m * 60;
	pt s,e; ri(s.fi, s.se, e.fi, e.se);
	vector<pt> v{s};
	pt x;
	while(cin >> x.fi >> x.se){
		v.push_back(x);
	...
	v.push_back(e);
	int N = v.size(),en = v.size() - 1;
	vvi G(N + 1);
	for(int i = 0; i < N; i++){
		auto [a, b] = v[i];
		for(int j = i + 1; j < N; j++){
			auto [c, d] = v[j];
			auto D = ((a - c) * (a - c) + (b - d) * (b - d));
			if(D >= lim*lim) continue;
			G[i].push_back(j); G[j].push_back(i);
		}
	}
	queue<int> Q;
	vi dist(N + 1, -1); dist[0] = 0;
	Q.push(0);
	while(Q.size()){
		auto cur = Q.front(); Q.pop();
		if(cur == en){
			po("Yes, visiting", dist[cur] - 1, "other holes.");
			return;
		}
