...
struct Fenwick{
    ...
    void Update(int i, int val){
        for(; i <= sz; i += i & -i) if((tree[i] += val) >= M) tree[i] -= M;
    }
    int Query(int i){
        int ret = 0;
        for(; i; i -= i & -i) if((ret += tree[i]) >= M) ret -= M;
        return ret;
    }
};
...
void Main(){
	int n,k; ri(n, k);
	vector<Fenwick> v(k, Fenwick(100'001));
	for(int i = 1; i <= n; i++){
		int t; ri(t); v[0].Update(t, 1);
		for(int j = 1; j < k; j++) v[j].Update(t, v[j - 1].Query(t - 1));
	}
	po(v[k - 1].Query(100'001));
} 
