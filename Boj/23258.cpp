...
void solve(){
    ri(n, q);
    for(int k = 0; k < 303; k++) for(int i = 0; i < 303; i++) for(int j = 0; j < 303; j++) D[k][i][j] = INF;
    for(int i = 1; i <= n; i++){
        for(int j = 1; j <= n; j++){
            int x; ri(x);
            if(!x) continue;
            D[i][j][0] = min<int>(D[i][j][0], x);
        }
    }
    for(int k = 1; k <= n; k++) for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++){
        D[i][j][k] = min<int>(D[i][j][k - 1], D[i][k][k - 1] + D[k][j][k - 1]);
    }
    while(q--){
        int c,s,e; ri(c, s, e);
        if(s == e) po(0);
        else po(D[s][e][c - 1] == INF ? -1 : D[s][e][c - 1]);
    }
}
