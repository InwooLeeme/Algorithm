...
void Move(char cur){
	const int kdx[9] = {1, 1, 1, 0, 0, 0, -1, -1, -1};
	const int kdy[9] = {-1, 0, 1, -1, 0, 1, -1, 0, 1};
    int d = cur - '0' - 1;
    auto nx = ix + kdx[d], ny = iy + kdy[d];
    if(Bound(nx, ny)) return;
    ix = nx;
    iy = ny;
}
...
void RMove(){
	...
	for(int i = 0; i < N; i++){
		...
		int mn = INF, idx = -1;
		for(int j = 0; j < 8; j++){
			auto nx = x + ddx[j], ny = y + ddy[j];
			if(Bound(nx, ny)) continue;
			auto D = abs(ix - nx) + abs(iy - ny);
			if(mn > D){
				mn = D;
				idx = j;
			}
		}
		auto nx = x + ddx[idx], ny = y + ddy[idx];
		if(Bound(nx, ny)) continue;
		mad[i] = {nx, ny};
	}
}
...
bool IsDead(){
	for(const auto& [x, y] : mad) if(x == ix and y == iy) return 1;
	return 0;
}
...
deque<pii> UpdateR(){
	deque<pii> DQ;
	map<pii, int> cnt;
	for(const auto& [x, y] : mad) cnt[{x, y}]++;
	for(const auto& [a, b] : cnt) if(b == 1) DQ.push_back(a);
	return DQ;
}

void PrintBoard(){
	board[ix][iy] = 'I';
	for(const auto& [x, y] : mad) board[x][y] = 'R';
	for(int i = 0; i < n; i++){
