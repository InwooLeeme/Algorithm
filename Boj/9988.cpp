...
void Main(){
	...
	int mx = 0;
	for(int i = 1; i < N; i++){
		auto a = cur[i - 1], b = cur[i];
		for(auto& [cost, y] : G[a]) if(y == b) cost <<= 1;
		auto [cdist, C] = Dijkstra(1, G);
		mx = max<int>(mx, abs(dist[n] - cdist[n]));
		for(auto& [cost, y] : G[a]) if(y == b) cost >>= 1;
	}
	po(mx);
}	
