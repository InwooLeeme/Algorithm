...
vi dijkstra(vector<vp>& g, int st){
	vi dist(n + 1, INF);
	minpq<pii> pq; pq.push({dist[st] = 0, st});
	while(si(pq)){
...
void solve(){
	while(cin >> n >> m >> s >> g1 >> g2){
		if(!n and !m and !s and !g1 and !g2) break;
		vector<vp> g(n + 1),rg(n + 1);
		for(int i = 0; i < m; i++){
			int a,b,c; ri(a, b, c);
			g[a].pb({c, b}); 
			rg[b].pb({c, a});
		...
		auto distg1 = dijkstra(rg, g1);
		auto distg2 = dijkstra(rg, g2);
		int res = INF;
		for(int i = 1; i <= n; i++){
			if(dists[i] == INF or distg1[i] == INF or distg2[i] == INF) continue;
			res = min<int>(res, dists[i] + distg1[i] + distg2[i]);
		}
		po(res);
	}
}
