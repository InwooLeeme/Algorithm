...
void Main(){ 
	...
	while(p--){
		int s,e; ri(s, e);
		vp dist(n + 1, {INF, INF}); //outdoor, shortestPath
		dist[s] = {0, 0};
		min_pq<ti3> PQ; PQ.push({dist[s].first, dist[s].second, s});
		while(PQ.size()){
			auto [outdoor, cdist, cur] = PQ.top(); PQ.pop();
			if(dist[cur].second < cdist) continue;
			for(const auto& [cost, state, nxt] : G[cur]){
				if(dist[nxt].first > outdoor + (state ? cost : 0)){
					dist[nxt].first = outdoor + (state ? cost : 0);
					dist[nxt].second = cdist + cost;
					PQ.push({dist[nxt].first, dist[nxt].second, nxt});
				}
				else if(dist[nxt].first == outdoor + (state ? cost : 0)){
					if(dist[nxt].second > cdist + cost){
						dist[nxt].second = cdist + cost;
						PQ.push({dist[nxt].first, dist[nxt].second, nxt});
					}
				}
		...
		if(dist[e].first == INF or dist[e].second == INF) po("IMPOSSIBLE");
		else po(dist[e].first, dist[e].second);
	}
} 
