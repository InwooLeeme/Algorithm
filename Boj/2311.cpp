...
void Main(){
    int n,m; ri(n, m); 
    auto in = [&](int x) -> int{ return x; };
    auto out = [&](int x) -> int{ return x + 1000; };
    int s = 0,e = 3332;
    MCMF mcmf(3333); mcmf.set_source(s); mcmf.set_sink(e);
    mcmf.add_edge(s, in(1), 2, 0); mcmf.add_edge(out(n), e, 2, 0);
    while(m--){
        int a,b,c; ri(a, b, c);
        mcmf.add_edge(out(a), in(b), 1, c); mcmf.add_edge(out(b), in(a), 1, c);
    }
    for(int i = 1; i <= n; i++) mcmf.add_edge(in(i), out(i), 2, 0);
