...
void Main(){
	int n,A; ri(n, A);
	vti3 v(n);
	for(auto& [a, b, c] : v) ri(a, b, c);
	auto Check = [&](const int mid) -> bool{
		int curH = mid, curA = A;
		for(const auto& [t, a, h] : v){
			if(t == 1){
				int enemyH = h,enemyA = a;
				curH -= (enemyH / curA + (enemyH % curA != 0) - 1) * a;
				if(curH <= 0) return 0;
			}
			else{
				curA += a; curH = (curH + h >= mid ? mid : curH + h);
			}
		}
		return curH > 0;
	};
	int lo = 0, hi = int(9e18);
	while(lo + 1 < hi){
		int m = lo + hi >> 1;
		if(!Check(m)) lo = m;
		else hi = m;
