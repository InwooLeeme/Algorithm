...
void Main(){
	ri(Q);
	list<pii> L;
	vector v(505051, L.end());
	L.insert(L.end(), {1, 1});
	L.insert(L.end(), {100001, 100001});
	auto H = [&](const string& s) -> int{
		int ret = 0;
		for(int i = 0; i < 4; i++){
			ret = 26 * ret + (s[i] - 'A');
		}
	...
	auto Insert = [&](int s, int sz) -> void{
		v[s] = L.end();
		auto cur = L.begin();
		auto nxt = next(cur);
		while(nxt != L.end() and nxt->first - cur->second < sz) cur++,nxt++;
		if(nxt == L.end()) return;
		v[s] = L.insert(nxt, {cur->second, cur->second + sz});
	};
	auto Free = [&](int s) -> void{
		if(v[s] == L.end()) return;
		L.erase(v[s]);
		v[s] = L.end();
	};
	...
	while(Q--){
		string s; ri(s);
		if(s[4] == '='){
			auto val = H(s.substr(0, 4));
			auto memorySize = stoi(s.substr(12, s.size() - 14));
			Insert(val, memorySize);
		}
		else if(s[0] == 'f'){
			auto val = H(s.substr(5, 4));
			Free(val);
		}
		else{
			auto val = H(s.substr(6, 4));
			P(val);
		}
	}
} 
