...
void DFS(int cur){
    ...
    for(const auto& nxt : G[cur]){
        if(indegree[nxt] or vist[nxt]) continue;
        Union(cur, nxt);
        DFS(nxt);
    }
}
...
void Main(){
    ...
    queue<int> Q;
    for(int i = 1; i <= N; i++) if(indegree[i] == 1) Q.push(i);
    while(SZ(Q)){
    ...
    for(int i = 1; i <= N; i++) if(indegree[i]) DFS(i);
    while(M--){
        int a,b; ri(a, b);
        po(Find(a) == Find(b) ? 1 : 2);
    }
}
