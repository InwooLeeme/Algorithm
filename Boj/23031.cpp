...
void solve(){
	...
	auto Fail = [&](int zx, int zy, int x, int y) -> bool{
		return (!On[zx][zy] and zx == x and zy == y);
	};
	int curx = 0,cury = 0,curd = 0;
	for(const auto& c : s){
		if(c == 'F'){
			auto nx = curx + kdx[curd]; 
			auto ny = cury + kdy[curd];
			if(!Bound(nx, ny)){
				curx = nx, cury = ny;
			}
		}
		...
		if(board[curx][cury] == 'S'){
			On[curx][cury] = 1;
			for(int d = 0; d < 8; d++){
				auto nx = curx + ddx[d], ny = cury + ddy[d];
				if(Bound(nx, ny)) continue;
				On[nx][ny] = 1;
				//debug(nx, ny);
			}
		}
		for(const auto& [x, y, dir] : Z){
			//debug(curx, cury, curd, x, y, dir, board[curx][cury], On[curx][cury]);
			if(Fail(x, y, curx, cury)){
				po("Aaaaaah!");
				return;
			}
		}
