...
int Sol(int x, int y){
	...
	if(ret != -1) return ret;
	ret = 0;
	if(v[x][y] == 'D') ret += Sol(x + 1, y);
	else if(v[x][y] == 'U') ret += Sol(x - 1, y);
	else if(v[x][y] == 'R') ret += Sol(x, y + 1);
	else if(v[x][y] == 'L') ret += Sol(x, y - 1);
	return ret;
}
...
void Main(){
	...
	ri(v);
	int cnt = 0;
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++) cnt += Sol(i, j);
	po(cnt);
}	
