...
int Conv(string s){
	...
	return ConvH(stoi(v[0]), stoi(v[1]));
}

void Main(){
	ri(n, m);
	gp_hash_table<string, int> T[(m / 7) + 1];
	gp_hash_table<string, int> cnt[(m / 7) + 1];
	int M = m / 7 + 1;
	vs v;
	for(int i = 0; i < n; i++){
		string name,a,b;int x; ri(name, x, a, b); --x;
		int s = Conv(a),e = Conv(b);
		int week = x / 7;
		T[week][name] += (e - s);
		cnt[week][name]++;
		v.push_back(name);
	...
	auto Check = [&](string s) -> bool{
		int ret = 0;
		for(int i = 0; i < M; i++){
			if(cnt[i][s] >= 5 and T[i][s] >= 60 * 60) ret++;
		}
		return ret == (m / 7);
	};
	for(const auto& i : v){
	...
	if(ret.empty()) po(-1);
	else for(const auto& i : ret) po(i);
} 
