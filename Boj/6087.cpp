...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
		if(v[i][j] == 'C'){
			if(st.first == -1 and st.second == -1) st = {i, j};
			else en = {i, j};
		}
	}
	auto Check = [&](int d, int nd) -> int{
		if((d == 0 or d == 2) and (nd == 1 or nd == 3)) return 1;
		if((d == 1 or d == 3) and (nd == 2 or nd == 0)) return 1;
		return 0;
	};
	auto Dijkstra = [&](int dir) -> int{
		min_pq<P> PQ; 
		vvi dist(n + 1, vi(m + 1, INF));
		PQ.push({dist[st.first][st.second] = 0, dir, st.first, st.second});
		while(PQ.size()){
			auto [cdist, cdir, x, y] = PQ.top(); PQ.pop();
			if(dist[x][y] < cdist) continue;
			if(x == en.first and y == en.second) return cdist;
			for(int d = 0; d < 4; d++){
				auto nx = x + dx[d], ny = y + dy[d];
				if(Bound(nx, ny) or v[nx][ny] == '*') continue;
				auto cost = Check(cdir, d);
				if(dist[nx][ny] > cdist + cost){
					PQ.push({dist[nx][ny] = cdist + cost, d, nx, ny});
				}
			}
		}
		return INF;
	...
	int mn = INF;
	for(int d = 0; d < 4; d++) mn = min<int>(mn, Dijkstra(d));
	po(mn);
}	
