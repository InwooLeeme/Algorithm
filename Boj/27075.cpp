...
vector<e> GetMst(vector<e>& E){
	UnionFind UF(2222);
	int cnt = 0;
	vector<e> ret;
	for(const auto& [c, i, a, b] : E){
		if(UF.Union(a, b)){
			ret.push_back({c, i, a, b});
			if(++cnt == n - 1) break;
		}
	}
	return ret;
...
void Main(){
	...
	for(int i = 0; i < (int)mst.size(); i++){
		auto [c, idx, a, b] = mst[i];
		mn = min<int>(mn, MST(E, idx));
	}
	po(mn);
} 
