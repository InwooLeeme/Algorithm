...
void Sol(){
	int n,q; ri(n, q);
	vvi v(1111, vi(1111)), p(1111, vi(1111));
	for(int i = 1; i <= n; i++) for(int j = 1; j <= n; j++) ri(v[i][j]);
	while(q--){
		int r1, c1, r2, c2, d; ri(r1, c1, r2, c2, d);
		p[r1][c1] += d;
		p[r2 + 1][c1] -= d;
		p[r1][c2 + 1] -= d;
		p[r2 + 1][c2 + 1] += d;
	...
	for(int i = 1; i <= 1000; i++){
		for(int j = 1; j <= 1000; j++) p[i][j] += p[i][j - 1];
	}
	for(int i = 1; i <= 1000; i++){
		for(int j = 1; j <= 1000; j++) p[j][i] += p[j - 1][i];
	}
	for(int i = 1; i <= n; i++){
		int ret = 0;
		for(int j = 1; j <= n; j++) ret += (p[i][j] + v[i][j]);
		cout << ret << " ";
	}
	cout << "\n";
	for(int i = 1; i <= n; i++){
		int ret = 0;
		for(int j = 1; j <= n; j++) ret += (p[j][i] + v[j][i]);
		cout << ret << " ";
	}
	cout << "\n";
}
