...
void Main(){
    ...
    vector<edge> E(m);
    for(int i = 0; i < m; i++){
        int a,b,c,t; ri(a, b, c, t);
        E[i] = {c, t, a, b};
    }
    sort(all(E), [&](edge& a, edge& b){
        return a[0] == b[0] ? C[a[1]] > C[b[1]] : a[0] < b[0];
    });
    UnionFind UF(n + 1);
    int cost = 0,cnt = 0;
    vector<p> H;
    for(int i = 0; i < m; i++){
        auto c = E[i][0], t = E[i][1], a = E[i][2], b = E[i][3];
        if(UF.Union(a, b)){
            H.pb({c, t});
            cost += c;
            if(++cnt == n - 1) break;
    ...
    vector<int> ret(3), Cnt(3);
    for(const auto& [a, b] : H){ ret[b] += a; Cnt[b]++;}
    po(cost);
    for(int i = 0; i < 3; i++) po(Cnt[i], ret[i]);
}
