...
void BFS(int r, int c, int idx){
	queue<pii> Q;
	Q.push({r, c}); dist[idx][r][c] = 0;
	while(Q.size()){
		auto [x, y] = Q.front(); Q.pop();
		for(int d = 0; d < 4; d++){
			auto nx = x, ny = y;
			auto nnx = x + dx[d], nny = ny + dy[d];
			int cnt = 0;
			while(1){
				if(Bound(nnx, nny) or v[nnx][nny] == -1) break;
				if(v[nnx][nny] == 7){
					nx = nnx;
					ny = nny;
					break;
			...
			if(cnt and dist[idx][nx][ny] == -1){
				dist[idx][nx][ny] = dist[idx][x][y] + 1; Q.push({nx, ny});
			}
		}
		for(int d = 0; d < 4; d++){
			auto nx = x + dx[d], ny = y + dy[d];
			if(Bound(nx, ny)) continue;
			if(dist[idx][nx][ny] != -1 or v[nx][ny] == -1) continue;
			dist[idx][nx][ny] = dist[idx][x][y] + 1; Q.push({nx, ny});
		}
	}
}
...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < n; j++){
		ri(v[i][j]);
		if(1 <= v[i][j] and v[i][j] <= 6) cod[v[i][j]] = {i, j};
	}
	int r,c; ri(r, c);
	cod[0] = {r, c};
	for(int i = 0; i <= 6; i++){
		BFS(cod[i].fi, cod[i].se, i);
	}
	vi w(6); iota(all(w), 1);
	int mn = INF;
	do{
		auto [sx, sy] = cod[w[0]];
		int ret = dist[0][sx][sy];
		if(ret == -1) continue;
		bool flag = 1;
		for(int i = 1; i < (int)w.size(); i++){
			auto [x, y] = cod[w[i - 1]];
			auto [ex, ey] = cod[w[i]];
			int c = dist[w[i - 1]][ex][ey];
			if(c == -1){
				flag = 0;
				break;
			}
