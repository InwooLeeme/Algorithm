...
void Main(){
	...
	vti3 Query;
	ri(q); for(int i = 0; i < q; i++){
		int t,a,b; ri(t);
		if(t == 1){
			ri(a); cod.push_back(a);
			Query.push_back({t, a, 0});
		}
		else{
			ri(a, b);
	...
	for(int i = 0; i < n; i++) FT.Update(IDX(v[i]), 1);
	for(auto& [t, a, b] : Query){
		if(t == 1){
			...
			int cnt = b - a + 1;
			int r = upper_bound(cod.begin(), cod.end(), b) - cod.begin() + 1;
			int l = IDX(a);
			po(cnt - (FT.Query(r - 1) - FT.Query(l - 1)));
		}
	}
}	
