...
int n,m,k;
int board[2020][2020], B[2020][2020], D[2020][2020];
...
void Main(){ 
	memset(D, -1, sizeof(D));
	ri(n, m, k);
	queue<ti3> M; queue<pii> Q;
	int x1 = -1,y1 = -1;
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
		ri(board[i][j]);
		if(board[i][j] == 4) { x1 = i, y1 = j; }
		else if(board[i][j] == 3) { B[i][j] = 1; M.push({k, i, j}); }
	}
	// Cal dist
	while(M.size()){
		auto [cdist, x, y] = M.front(); M.pop();
		for(int d = 0; d < 4; d++){
			auto nx = x + dx[d], ny = y + dy[d];
			auto ndist = cdist - 1;
			if(Bound(nx, ny) or ndist < 0 or B[nx][ny]) continue;
			B[nx][ny] = 1; M.push({ndist, nx, ny});
		}
	}
	B[x1][y1] = 0;
	D[x1][y1] = 0; Q.push({x1, y1});
	while(Q.size()){
		...
		for(int d = 0; d < 4; d++){
			auto nx = x + dx[d], ny = y + dy[d];
			if(Bound(nx, ny) or D[nx][ny] != -1 or board[nx][ny] == 1 or B[nx][ny]) continue;
