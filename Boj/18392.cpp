...
void Main(){
	auto Sol = [&]() -> void{
		...
		string s; ri(s);
		for(auto& i : s) if(i == ',') i = ' ';
		string temp; stringstream in(s);
		vs v; while(in >> temp) v.push_back(temp);
		vi c;
		for(auto& i : v){
			for(auto& j : i) if(j == ':') j = ' ';
			string a,b; stringstream cur(i);
			cur >> a >> b; 
			int x = stoi(a),y = stoi(b);
			while(y--) c.push_back(x);
		}
		dsort(c);
		...
		if(!N) for(const auto& [a, b] : M) cout << a << " " << b << "\n";
		else po("Impossible");
	};
	int t; ri(t); for(int tc = 1; tc <= t; tc++){
		cout << "Customer" << tc << ":\n";
		Sol();
	}
} 		
