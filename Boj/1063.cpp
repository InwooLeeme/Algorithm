...
const int kdx1[4] = {-1, 1, 0, 0};
const int kdy1[4] = {0, 0, -1, 1};
const int kdx2[4] = {-1, 1, -1, 1};
const int kdy2[4] = {1, 1, -1, -1};
const int N = 8;
...
string i2c(int x, int y){
	return string("ABCDEFGH")[x] + to_string(y + 1);
}

void Main(){
	string a,b; int q; ri(a, b, q);
	auto [sx, sy] = c2i(a);
	auto [ex, ey] = c2i(b);
	auto I = [&](string s) -> int{
		if(s == "LT") return 0;
		else if(s == "RT") return 1;
		else if(s == "LB") return 2;
		return 3;
	};
	while(q--){
		string s; ri(s);
		if((int)s.size() == 1){
			auto d = string("LRBT").find(s[0]);
			auto nsx = sx + kdx1[d], nsy = sy + kdy1[d];
			auto nex = ex, ney = ey;
			bool flag = 0;
			if(nsx == ex and nsy == ey) {nex += kdx1[d], ney += kdy1[d]; flag = 1;}
			if(Bound(nsx, nsy) or Bound(nex, ney)) continue;
			if(flag) ex = nex, ey = ney;
			sx = nsx, sy = nsy;
		}
		else{
			auto d = I(s);
			auto nsx = sx + kdx2[d], nsy = sy + kdy2[d];
			auto nex = ex, ney = ey;
			bool flag = 0;
			if(nsx == ex and nsy == ey) {nex += kdx2[d], ney += kdy2[d]; flag = 1;}
			if(Bound(nsx, nsy) or Bound(nex, ney)) continue;
			if(flag) ex = nex, ey = ney;
			sx = nsx, sy = nsy;
		}
