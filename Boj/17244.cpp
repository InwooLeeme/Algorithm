...
void Main(){
    ...
    auto toggle = [&](int k, int x) -> int{ return (k | (1 << x)); };
    auto Have = [&](int k, int x) -> int{ return (k & (1 << x)); };
    int ex = -1, ey = -1;
    queue<I> Q; int bit = 0,xcnt = 0;
    for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
        if(board[i][j] == 'S'){
            dist[i][j][0] = 0; Q.push({0, 0, i, j});
        }
        else if(board[i][j] == 'E') ex = i, ey = j;
        else if(board[i][j] == 'X'){ 
            board[i][j] = char(xcnt + '0');
            bit |= (1 << xcnt); xcnt++;
        }
    }
    while(Q.size()){
        auto [state, cnt, x, y] = Q.front(); Q.pop();
        if(x == ex and y == ey and state == bit){
            po(dist[x][y][state]);
            return;
        }
        for(int d = 0; d < 4; d++){
            auto nx = x + dx[d], ny = y + dy[d];
            if(Bound(nx, ny) or board[nx][ny] == '#' or dist[nx][ny][state] != -1) continue;
            if('0' <= board[nx][ny] and board[nx][ny] <= '4'){
                auto nstate = (!Have(state, board[nx][ny] - '0') and state < bit) ? toggle(state, board[nx][ny] - '0') : state;
                auto ncnt = (!Have(state, board[nx][ny] - '0') and state < bit) ? cnt + 1 : cnt;
                dist[nx][ny][nstate] = dist[x][y][state] + 1;
                Q.push({nstate, ncnt, nx, ny});
            }
            if(board[nx][ny] == '.' or board[nx][ny] == 'E' or board[nx][ny] == 'S'){
                dist[nx][ny][state] = dist[x][y][state] + 1; 
                Q.push({state, cnt, nx, ny});
            }
        }
    }
} 		
