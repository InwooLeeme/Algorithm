...
struct BipMatching{
    ...
    vector<vector<int>> g;
    vector<int> par,vist;
    BipMatching(int n):n(n),vist(n + 1),par(n + 1, -1),g(n + 1){}
...
int32_t main(){
    ...
    int n; sf1(n);
    vector<pii> a(n),b(n);
    for(auto& [x,y] : a) sf2(x, y);
    for(auto& [x,y] : b) sf2(x, y);
    ...
    while(lo + 1 < hi){
        ...
        for(int i = 0; i < n; i++){
            for(int j = 0; j < n; j++){
                auto A = a[i],B = b[j];
                auto dist = abs(A.first - B.first) + abs(A.second - B.second);
                if(dist <= mid) G.AddEdge(i, j);
            }
        }
        if(G.Matching() != n) lo = mid;
        else hi = mid;
    }
    pf1l(hi);
    return 0;
