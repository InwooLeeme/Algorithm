...
auto Sol = [](){
	int n; vi v(7); ri(n); 
	for(int i = 1; i <= 6; i++) ri(v[i]);
	vvi C(4, vi(4));
	for(int i = 1; i <= 3; i++) for(int j = 1; j <= 3; j++) ri(C[i][j]);
	MinCostFlow<int, int> g(20);
	int S = 0, E = 15;
	for(int i = 1; i <= 3; i++) g.add_edge(S, i, v[i], 0);
	for(int i = 4; i <= 6; i++) g.add_edge(i, E, v[i], 0);
	for(int i = 1; i <= 3; i++){
		for(int j = 1; j <= 3; j++){
			g.add_edge(i, j + 3, INF, -C[i][j]);
		}
	}
	auto [f, c] = g.run(S, E);
	po(-c);
