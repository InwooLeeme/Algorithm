...
int board[1010][1010], dist[1010][1010];

bool Bound(int x, int y){ return (x < 0 or x + h - 1 >= n or y < 0 or y + w - 1 >= m); }
...
void Main(){
	...
	ri(h, w, sr, sc, fr, fc); --sr,--sc,--fr,--fc;
	auto Collision = [&](int x, int y) -> bool{
		for(const auto& [a, b] : v){
			if((x <= a and a <= x + h - 1) and (y <= b and b <= y + w - 1)) return 1;
		}
		return 0;
	};
	queue<pii> Q; Q.push({sr, sc}); dist[sr][sc] = 0;
	while(Q.size()){
		...
		for(int d = 0; d < 4; d++){
			...
			if(Bound(nx, ny) or dist[nx][ny] != -1 or Collision(nx, ny)) continue;
