...
void Main(){ 
	...
	[&](){
		for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
			if(board[i][j] == '2'){
				PQ.push({dist[i][j] = 0, 0, 0, i, j});
				return;
			}
		}
	}();
	while(PQ.size()){
		auto [cdist, lstep, rstep, x, y] = PQ.top(); PQ.pop();
		if(cdist < dist[x][y]) continue;
		for(int d = 0; d < 4; d++){
			auto nx = x + dx[d], ny = y + dy[d];
			auto cost = ((d == 0 or d == 2) ? 1 : 0);
			auto nlstep = lstep + (d == 0);
			auto nrstep = rstep + (d == 2);
			if(Bound(nx, ny) or board[nx][ny] == '1') continue;
			if(nlstep > L or nrstep > R) continue;
			if(dist[nx][ny] > cdist + cost){
				PQ.push({dist[nx][ny] = cdist + cost, nlstep, nrstep, nx, ny});
			}
		}
	}
	int ret = 0;
	for(int i = 0; i < n; i++){
		for(int j = 0; j < m; j++) ret += (dist[i][j] != INF);
	}
	po(ret);
} 
