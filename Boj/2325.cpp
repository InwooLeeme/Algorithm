...
pair<vi, vi> Dijkstra(vector<vp>& G, int s = 1){
	min_pq<pii> PQ;
...
void Main(){
	...
	int mx = 0;
	for(int i = 1; i < N; i++){
		auto a = bk[i - 1],b = bk[i];
		int temp = 0;
		for(auto& [cost, y] : G[a]) if(y == b){
			temp = cost;
			cost = INF;
		}
		for(auto& [cost, y] : G[b]) if(y == a) cost = INF;					
		auto [cdist, cpath] = Dijkstra(G);
		for(auto& [cost, y] : G[a]) if(y == b) cost = temp;
		for(auto& [cost, y] : G[b]){
			if(y == a) cost = temp;			
		}
		mx = max<int>(mx, cdist[n]);
	}
	po(mx);
}	
