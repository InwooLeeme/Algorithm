...
void Main(){
	int n,m;
	while(cin >> n >> m and n){
		auto in = [&](int x) -> int{ return x + 10000; };
		vector<Fenwick> v(m, Fenwick(20022));
		for(int i = 1; i <= n; i++){
			int t; ri(t);
			auto x = in(t); v[0].Update(x, 1);
			for(int j = 1; j < m; j++) v[j].Update(x, v[j - 1].Query(x - 1));
		}
		po(v[m - 1].Query(20000));
	}
}	
