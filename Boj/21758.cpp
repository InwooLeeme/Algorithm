...
void Main(){
	...
	int mx = -INF;
	for(int i = 2; i < n; i++) mx = max<int>(mx, P[n] - P[1] + P[n] - P[i] - v[i]);
	for(int i = 2; i < n; i++) mx = max<int>(mx, P[i - 1] + (P[n - 1] - v[i]));
	for(int i = 2; i < n; i++) mx = max<int>(mx, P[i] - P[1] + P[n - 1] - P[i - 1]);
	po(mx);
}
