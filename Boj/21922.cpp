...
int n,m,k;
int dist[2020][2020], D[2020][2020], board[2020][2020];
...
void Main(){ 
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
		...
		else if(board[i][j] == 3) I.push({k, i, j}), D[i][j] = 1;
	}
	while(I.size()){
		auto [cdist, x, y] = I.front(); I.pop();
		if(!cdist) continue;
		for(int d = 0; d < 4; d++){
			auto nx = x + dx[d], ny = y + dy[d];
			if(Bound(nx, ny) or D[nx][ny] or board[nx][ny] == 1) continue;
			D[nx][ny] = 1; I.push({cdist - 1, nx, ny});
		}
	}
	D[x1][y1] = 0;
	queue<pii> Q; dist[x1][y1] = 0; Q.push({x1, y1});
	while(Q.size()){
		auto [x, y] = Q.front(); Q.pop();
		if(D[x][y] and board[x][y] != 4) continue;
		if(board[x][y] == 2){
			po(dist[x][y]);
			return;
		}
