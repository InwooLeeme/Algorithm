...
void Sol(){
	...
	for(int i = 0; i < k; i++){
		...
		auto x = n - arrow[i];
		while(1){
			if(Bound(x, y) or board[x][y] == 'x') break;
			y += d;
		}
		board[x][y] = '.';
		memset(vist, 0, sizeof(vist));
		for(int j = 0; j < m; j++){
			if(vist[n - 1][j] or board[n - 1][j] == '.') continue;
			queue<pii> Q; Q.push({n - 1, j}); vist[n - 1][j] = 1;
			while(!Q.empty()){
				auto& [x, y] = Q.front(); Q.pop();
				for(int d = 0; d < 4; d++){
					auto nx = x + dx[d], ny = y + dy[d];
					if(Bound(nx, ny) or vist[nx][ny] or board[nx][ny] == '.') continue;
		...
		for(int j = 0; j < m; j++){
			int idx = n - 1;
			while(idx >= 0 and (board[idx][j] == '.' or vist[idx][j])) idx--;
			if(idx < 0) continue;
			int idx2 = idx + 1;
			while(idx2 < n and !vist[idx2][j]) idx2++;
			chmin(H, idx2 - idx - 1);
		}
		if(H == INF) continue;

		for(int j = m - 1; j >= 0; j--) for(int i = n - 1; i >= 0; i--){
			if(board[i][j] == '.' or vist[i][j]) continue;
			board[i + H][j] = 'x';
			vist[i + H][j] = 1;
			board[i][j] = '.';
		}
