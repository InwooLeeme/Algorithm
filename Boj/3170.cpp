...
int r,c,n,k;
auto bound = [](int x, int y){ return x < 0 or x > r or y < 0 or y > c; };
...
void solve(){
	ri(r, c, n);
	vector<ti3> v(n); for(auto& [a, b, c] : v) ri(a, b, c);
	ri(k);
	vp query(k); for(auto& [a, b] : query) ri(a, b);
	int S = n + 1;
	uf UF(S + 1);
	int sr = r / 2, sc = 0;
	for(int i = 0; i < n; i++){
		auto [a, b, c] = v[i];
		auto fx = a - sr, fy = b - sc;
		if(fx*fx + fy*fy <= c*c) UF.Union(i, S);
	}	
	for(int i = 0; i < n; i++){
		auto [x1, y1, r1] = v[i];
        for(int j = i + 1; j < n; j++){
			auto [x2, y2, r2] = v[j];
            auto fx = (x1 - x2), fy = (y2 - y1);
            auto R = r1 + r2;
            if(fx * fx + fy * fy <= R*R) UF.Union(i, j);
        }
    }
	int root = UF.Find(S);
	for(const auto& [a, b] : query){
		bool ok = false;
        if(a == sr and b == sc) ok = 1;
        else{
            for(int j = 0; j < n and !ok; j++){
                if(UF.Find(j) != root) continue;
				auto [x1, y1, r] = v[j];
                auto fx = x1 - a, fy = y1 - b;
                if(fx * fx + fy * fy <= r * r) ok = 1;
            }
        }
        po((ok ? "DA" : "NE"));
	}
