...
void solve(){
	...
	vvi D(n + 1, vi(2));
	auto sol = [&](int cur, auto&& sol) -> void{
		D[cur][0] = D[cur][1] = INF;
		if(cur == T){
			D[cur][0] = v[cur]; 
			return;
		...
		for(const auto& [cost, nxt] : g[cur]){
			...
			if(!D[nxt][0]) sol(nxt, sol);
			if(D[nxt][0] == INF) continue;
			D[cur][0] = min(D[cur][0], min<int>(v[nxt], D[nxt][0]));
			D[cur][1] = min(D[cur][1], D[nxt][1]);
		}
		D[cur][1] = min<int>(D[cur][1], v[cur] + D[cur][0]);
	};
	sol(S, sol);
	po(D[S][1]);
}
