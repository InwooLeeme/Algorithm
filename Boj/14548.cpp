...
void Sol(){
    ...
    sf1(n); cin.ignore();
    string st,en; sf2(st, en);
    vector<pii> adj[511];
    for(int i = 0,t; i < n; i++){
        string x,y; sf3(x, y, t);
        if(!M.count(x)) M[x] = idx++;
        if(!M.count(y)) M[y] = idx++;
        adj[M[x]].push_back({t, M[y]});
        adj[M[y]].push_back({t, M[x]});
    }
    vector<int> dist(511, INF);
    min_heap<pii> PQ;
    dist[M[st]] = 0;
    PQ.push({dist[M[st]],M[st]});
    while(!PQ.empty()){
