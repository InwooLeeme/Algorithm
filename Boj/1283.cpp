...
void Main(){ 
	...
	map<char, int> M;
	auto Sol = [&](string& s) -> void{
		auto ss = Split(s);
		int r = -1, c = -1;
		for(int i = 0; i < ss.size(); i++){
			if(!M.count(toupper(ss[i][0])) and !M.count(tolower(ss[i][0]))){
				M[ss[i][0]] = 1;
				r = i,c = 0;
				break;
			}
		...
		if(r != -1){ Print(); }
			...
			for(int i = 0; i < ss.size(); i++){
				for(int j = 0; j < ss[i].size(); j++){
					if(!M.count(toupper(ss[i][j])) and !M.count(tolower(ss[i][j]))){
						M[ss[i][j]] = 1;
						r = i, c = j;
						break;
					}
