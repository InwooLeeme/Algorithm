		...
		if(v[i][j] == 'X' and !vist[i][j]){
			...
			while(Q.size()){
				...
				for(int d = 0; d < 4; d++){
					...
					if(vist[nx][ny] or v[nx][ny] != 'X') continue;
					vist[nx][ny] = 1; Q.push({nx, ny});
				}
			}
			C++;
		}
...
void Main(){
	...
	for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
		if(idx[i][j] != -1){
			G[idx[i][j]].push_back({i, j});
			mx = max<int>(mx, idx[i][j]);
		}
	}
	int N = mx + 1;
	// Get Dist
		...
		for(int j = i + 1; j <= mx; j++){
			...
			while(Q.size()){
				...
				for(int d = 0; d < 4; d++){
					auto nx = x + dx[d], ny = y + dy[d];
					if(Bound(nx, ny)) continue;
					if(idx[nx][ny] == j) mndist = min<int>(mndist, dist[x][y]);
					else if(v[nx][ny] == 'S' and dist[nx][ny] > dist[x][y] + 1){
						dist[nx][ny] = dist[x][y] + 1;
						Q.push({nx, ny});
					}
				}
			}
			cost[i][j] = cost[j][i] = mndist;
		}
	}
	// floyd
	...
	// TSP
	vvi D(N + 1, vi(1 << (N + 1), -1));
	auto TSP = [&](int cur, int vist, auto&& TSP) -> int{
		...
		for(int i = 0; i < N; i++){
			...
			if(cost[cur][i] == INF) continue;
			ret = min<int>(ret, TSP(i, vist | (1 << i), TSP) + cost[cur][i]);
		}
		return ret;
	};
	int mn = INF;
	for(int i = 0; i < N; i++){
		mn = min<int>(mn, TSP(i, 1 << i, TSP));
	}
	po(mn);
}	
