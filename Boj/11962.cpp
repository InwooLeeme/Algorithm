...
template<size_t _sz>
struct SegTree{
	vector<int> tree,mntree,lazy;

	SegTree():tree(_sz << 1), mntree(_sz << 1), lazy(_sz << 1){}

	void Push(int node, int L, int R){
		if(node < _sz) for(const auto nxt : {node << 1, node << 1 | 1}) lazy[nxt] += lazy[node];
		tree[node] += lazy[node] * (R - L + 1);
		mntree[node] += lazy[node];
		lazy[node] = 0;
	}

	void Update(int l, int r, int val, int node = 1, int L = 1, int R = _sz){
		Push(node, L, R);
		...
		mntree[node] = min<int>(mntree[node << 1], mntree[node << 1 | 1]);
	}

	int SumQuery(int l, int r, int node = 1, int L = 1, int R = _sz){
		Push(node, L, R);
		if(r < L or R < l) return 0;
		if(l <= L && R <= r) return tree[node];
		int mid = L + R >> 1;
		return SumQuery(l, r, node << 1, L, mid) + SumQuery(l, r, node << 1 | 1, mid + 1, R);
	}

	int MnQuery(int l, int r, int node = 1, int L = 1, int R = _sz){
		Push(node, L, R);
		if(r < L or R < l) return INF;
		if(l <= L && R <= r) return mntree[node];
		int mid = L + R >> 1;
		return min<int>(MnQuery(l, r, node << 1, L, mid), MnQuery(l, r, node << 1 | 1, mid + 1, R));
	}
};
...
void Main(){
	...
	while(m--){
		char c; int s,e; ri(c, s, e);
		if(c == 'M') po(ST.MnQuery(s, e));
		else if(c == 'S') po(ST.SumQuery(s, e));
		else{
			int x; ri(x);
			ST.Update(s, e, x);
		}
