...
void Main(){
	...
	min_pq<I> PQ; vvi dist(n + 1, vi(m + 1, INF));
	dist[0][0] = 0; PQ.push({dist[0][0], k, k, 0, 0});
	while(PQ.size()){
		auto [cdist, l, r, x, y] = PQ.top(); PQ.pop();
		if(dist[x][y] < cdist) continue;
		if(x == n - 1 and y == m - 1){
			po("Yes");
			return;
		}
		auto cd = string("URDL").find(board[x][y]);
		for(const auto& d : {(cd - 1) % 4, cd, (cd + 1) % 4}){
			auto nx = x + kdx[d], ny = y + kdy[d];
			auto cost = (d == cd ? 0 : 1);
			auto nl = (d == ((cd - 1) % 4) ? l - 1 : l);
			auto nr = (d == ((cd + 1) % 4) ? r - 1 : r);
			if(Bound(nx, ny)) continue;
			if(d == ((cd - 1) % 4) and !l) continue;
			if(d == ((cd + 1) % 4) and !r) continue;
			if(dist[nx][ny] > cdist + cost){
				dist[nx][ny] = cdist + cost;
				PQ.push({dist[nx][ny], nl, nr, nx, ny});
			}
		}
	}
	po("No");
