...
void Main(){
	...
	sort(v.begin(), v.end(), [&](string& a, string& b){
		auto an = a.substr(0, a.find('.')), bn = b.substr(0, b.find('.'));
		string aos = a.substr(a.find('.') + 1), bos = b.substr(b.find('.') + 1);
		if(an == bn){
			if(!S.count(aos) and !S.count(bos)) return aos < bos;
			else if(S.count(aos) and !S.count(bos)) return a > b;
			else if(!S.count(aos) and S.count(bos)) return a < b;
			else if(S.count(aos) and S.count(bos)) return aos < bos;
		}
		return an < bn;
	});
	for(auto& i : v) po(i);
