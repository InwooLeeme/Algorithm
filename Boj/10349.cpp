...
int n,m,t,l,w;
int board[1111][1111], dist[1111][1111];
...
void Main(){
	while(cin >> n and n != -1){
		...
		for(int i = 0; i < w; i++){
			...
			if(x1 == x2){
				if(y1 > y2) swap(y1, y2);
				for(int c = y1; c <= y2; c++) board[x1][c] = 1;
			}
			else if(y1 == y2){
				if(x1 > x2) swap(x1, x2);
				for(int r = x1; r <= x2; r++) board[r][y1] = 1;
			}
			else{
				auto fx = (x1 - x2 == 0 ? 0 : (x1 - x2 > 0 ? -1 : 1));
				auto fy = (y1 - y2 == 0 ? 0 : (y1 - y2 > 0 ? -1 : 1));
				int nx = x1, ny = y1;
		...
		while(Q.size()){
			...
			for(int d = 0; d < 4; d++){
				...
				if(dist[nx][ny] != -1 or board[nx][ny]) continue;
				dist[nx][ny] = dist[x][y] + 1; Q.push({nx, ny});
			}
		}
		int ret = 0;
		for(int i = 0; i < n; i++) for(int j = 0; j < m; j++) ret += (dist[i][j] <= t and dist[i][j] != -1 and !board[i][j]);
		po(ret);
	}
}
