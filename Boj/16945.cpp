...
bool Check(vvi& v){
    ...
    for(int i = 0; i < 3; i++){
        int t1 = 0,t2 = 0;
        for(int j = 0; j < 3; j++) t1 += v[i][j];
        for(int j = 0; j < 3; j++) t2 += v[j][i];
        M[t1]++; M[t2]++;
    }
    M[(v[0][0] + v[1][1] + v[2][2])]++; M[(v[0][2] + v[1][1] + v[2][0])]++;
    return (int)M.size() == 1;
}

void Main(){
	for(int i = 0; i < 3; i++) for(int j = 0; j < 3; j++) ri(board[i][j]);
    vi v(9); iota(all(v), 1);
    int mn = INF;
    do{
        vvi w(3, vi(3));
        ...
        if(!Check(w)) continue;
        int cur = 0;
        for(int i = 0; i < 3; i++) for(int j = 0; j < 3; j++) cur += abs(board[i][j] - w[i][j]);
        mn = min<int>(mn, cur);
