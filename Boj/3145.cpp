...
void Main(){
    ...
    int cnt1 = 0,cnt2 = 0;
    vvi v(55, vi(55)),w(55, vi(55));
    for(int i = 0; i < n; i++) for(int j = 0; j < m; j++){
        if('A' <= board[i][j] and board[i][j] <= 'Z' and !v[i][j]){
            cnt1++;
            v[i][j] = cnt1;
            string cur = "";
            while('A' <= board[i][j] and board[i][j] <= 'Z'){
                cur += board[i][j];
                v[i][j] = cnt1;
                j++;
            }
    ...
    for(int i = 1; i <= cnt2; i++) po(I[i].fi + 1, I[i].se + 1, M[r[i]]);    
}
