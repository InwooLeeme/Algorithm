...
void Main(){
	int t; ri(t);
	I[1] = I[2] = 1;
	for(int i = 3; i <= sz; i++) I[i] = I[i - 1] + I[i - 2];
	set<int> f,s,e;
	for(int i = 1; i <= sz; i++) f.insert(I[i]);
	for(int i = 1; i <= sz; i++){
		for(int j = 1; j <= sz; j++) s.insert(I[i] + I[j]);
	}
	for(int i = 1; i <= sz; i++) for(int j = 1; j <= sz; j++) for(int k = 1; k <= sz; k++){
		e.insert(I[i] + I[j] + I[k]);
	}
	while(t--){
		int k,x; ri(k, x);
		if(k == 1) po(f.count(x) ? "YES" : "NO");
		else if(k == 2) po(s.count(x) ? "YES" : "NO");
		else if(k == 3) po(e.count(x) ? "YES" : "NO");
	}
}	
