...
void Main(){
	...
	while(a--){
		string s; int x; ri(s, x); normal[s] = x;
	}
	while(b--){
		string s; int x; ri(s, x); special[s] = x;
	}
	while(c--){
		string s; int x; ri(s); service[s] = 1;
	}
	int q,ncur = 0,scur = 0,sscnt = 0; ri(q); while(q--){
		string s; ri(s);
		if(normal.count(s)) ncur += normal[s];
		else if(special.count(s)) scur += special[s];
		else if(service.count(s)) sscnt++;
	}
	if(ncur < 20000 and scur) po("No");
	else if(ncur + scur < 50000 and sscnt) po("No");
	else if(sscnt >= 2) po("No");
	else po("Okay");
}	
