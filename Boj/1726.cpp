...
int n,m;
int board[111][111], dist[111][111][4];
...
void Main(){
    ...
    int sx,sy,sd,ex,ey,ed; ri(sx, sy, sd, ex, ey, ed); --sx,--sy,--ex,--ey;
    sd = I(sd), ed = I(ed);
    queue<P> Q; Q.push({dist[sx][sy][sd] = 0, sd, sx, sy});
    while(Q.size()){
        auto [cdist, cd, x, y] = Q.front(); Q.pop();
        if(x == ex and y == ey and cd == ed){
            po(cdist);
            return;
        }
        for(const auto& d : {-1, 1}){
            auto nx = x, ny = y, nd = (cd + d + 4) % 4;
            if(dist[nx][ny][nd] != -1) continue;
            dist[nx][ny][nd] = cdist + 1; Q.push({dist[nx][ny][nd], nd, nx, ny});
            
        }
        for(int k = 1; k <= 3; k++){
            auto nx = x + k* dx[cd], ny = y + k * dy[cd]; 
            if(Bound(nx, ny) or dist[nx][ny][cd] != -1) continue;
            if(board[nx][ny]) break;
            dist[nx][ny][cd] = cdist + 1; Q.push({dist[nx][ny][cd], cd, nx, ny});            
        }
    }
} 		
