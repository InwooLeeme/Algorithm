...
void Main(){
	ri(n, k, t);
	int px,py,pv; ri(px, py, pv);
	vector<ti3> w;
	vi have;
	for(int i = 0; i < n; i++){
		int a,b,c,d; ri(a, b, c, d);
		w.push_back({a, b, c});
		if(d) have.push_back(i);
	}
	// construct Graph
	for(int i = 0; i < n; i++){
		auto [a, b, c] = w[i];
		auto D = (a - px) * (a - px) + (b - py) * (b - py);
		if(abs(pv - c) <= t and D <= k*k){
			G[i].push_back(n + 1);
			G[n + 1].push_back(i);
		}
	}
	...
	dfs(n + 1);
	vi ret;
	for(const auto& i : have) if(vist[i]) ret.push_back(i + 1);
	if(ret.empty()) po(0);
	else for(const auto& i : ret) cout << i << " ";
}
