...
void Main(){
    ...
    auto D = [](int a, int b, int c, int d) -> int{ return (a - c) * (a - c) + (b - d) * (b - d); };
    vector<ti3> E;
    UnionFind UF(n);
    for(const auto& [a, b] : I){
        for(int i = 0; i <= 10; i++){
            auto it = lower_bound(all(Y[i]), a.fi);
            for(int j = -1; j <= (i == a.se); j++){
                if(it + j < begin(Y[i]) or it + j >= end(Y[i])) continue;
                p nxt(it[j], i);
                E.pb({D(a.fi, a.se, nxt.fi, nxt.se), I[nxt], I[a]});
            }
        }
    }
    sort(all(E));
