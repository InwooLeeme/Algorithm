...
int n,m,k;
int v[1010][1010], vist[1010][1010];
...
void Main(){ 
	ri(n, m, k);
	int idx = 0, x = 1, y = 1,dir = 0,retx = 0,rety = 0;
	while(1){
		if(idx == n * m) break;
		if(vist[x + kdx[dir]][y + kdy[dir]] or Bound(x + kdx[dir], y + kdy[dir])) dir = (dir + 1) % 4;
		vist[x][y] = 1;
		v[x][y] = ++idx;
		if(idx == k) retx = x, rety = y;
		x += kdx[dir]; y += kdy[dir];
	}
	if((n * m) < k) po(0);
	else po(retx, rety);
