...
int n,m;
vector<pair<double, int>> adj[1111];
pii cod[1111];
...
int32_t main(){
    ...
    for(int i = 1; i <= n; i++){
        for(int j = i + 1; j <= n; j++){
            auto s = cod[i],e = cod[j];
            double d = sqrt((s.first - e.first) * (s.first - e.first) + (s.second - e.second) * (s.second - e.second));
            if(d > k) continue;
            adj[i].push_back({d, j});
            adj[j].push_back({d, i});
        }
