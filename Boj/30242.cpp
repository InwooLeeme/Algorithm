...
int n, k;
int H1[22], H2[44], H3[222];
int check1[22], check2[44], check3[222];
vector<int> v,ret(22, -1);
...
void f(int dep){
    ...
    int x = v[dep];
    for(int y = 0; y < n; y++){
        if(check1[y] or check2[x + y] or check3[x - y + n - 1]) continue;
        if(H1[y] or H2[x + y] or H3[x - y + n - 1]) continue;
        H1[y] = x + 1;
        H2[x + y] = x + 1;
        H3[x - y + n - 1] = x + 1;
        f(dep + 1);
