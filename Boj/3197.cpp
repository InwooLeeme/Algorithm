...
void Main(){ 
    ...
    while(Q.size()){
        ...
        for(int d = 0; d < 4; d++){
            ...
            if(Bound(nx, ny) or D[nx][ny] != -1) continue;
            if(v[nx][ny] != 'X') continue;
            D[nx][ny] = D[x][y] + 1; Q.push({nx, ny});
        }
    }
    auto Check = [&](const int mid) -> bool{ // 현재 날짜가 mid일때 다른 백조를 만날 수 있는가?
        memset(dist, 0, sizeof(dist));
        queue<pii> Q; Q.push({w[0].first, w[0].second});
        dist[w[0].first][w[0].second] = 1;
        while(Q.size()){
            auto [x, y] = Q.front(); Q.pop();
            if(x == w[1].first and y == w[1].second) return 0;
            for(int d = 0; d < 4; d++){
                auto nx = x + dx[d], ny = y + dy[d];
                if(Bound(nx, ny) or dist[nx][ny]) continue;
                if(D[nx][ny] > mid) continue;
                dist[nx][ny] = 1; Q.push({nx, ny});
            }
        }
        return 1;
    };  
