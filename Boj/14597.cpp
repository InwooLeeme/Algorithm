...
vvi D(111, vi(111, INF));

bool Bound(int x, int y){ return x < 1 or x > n or y < 1 or y > m; }
int F(int x){ return x * x; }

void Main(){ 
	ri(n, m);
	for(int i = 1; i <= n; i++) for(int j = 1; j <= m; j++) ri(A[i][j]);
	for(int i = 1; i <= n; i++) for(int j = 1; j <= m; j++) ri(B[i][j]);
    for(int j = 1; j <= m; j++) D[1][j] = min<int>(D[1][j], F(A[1][j] - B[1][j]));
	for(int i = 2; i <= n; i++){
		for(int j = 1; j <= m; j++){
			if(!Bound(i - 1,j + 1)) D[i][j] = min<int>(D[i][j], D[i - 1][j + 1] + F(A[i][j] - B[i][j]));
			if(!Bound(i - 1, j)) D[i][j] = min<int>(D[i][j], D[i - 1][j] + F(A[i][j] - B[i][j]));
			if(!Bound(i - 1, j - 1)) D[i][j] = min<int>(D[i][j], D[i - 1][j - 1] + F(A[i][j] - B[i][j]));
		}
	}
    int mn = INF;
	for(int j = 1; j <= m; j++) mn = min<int>(mn, D[n][j]);
	po(mn);
} 		
