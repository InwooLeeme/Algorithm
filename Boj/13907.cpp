...
void Main(){
	...
	min_pq<ti3> PQ;
	D[s][0] = 0; PQ.push({D[s][0] = 0, 0, s}); // cdist, cnt, node
	while(PQ.size()){
		auto [cdist, cnt, cur] = PQ.top(); PQ.pop();
		if(D[cur][cnt] < cdist) continue;
		for(const auto& [cost, nxt] : G[cur]){
			auto ncnt = cnt + 1;
			if(ncnt > n) continue;
			if(D[nxt][ncnt] > cdist + cost){
				D[nxt][ncnt] = cdist + cost;
				PQ.push({D[nxt][ncnt], ncnt, nxt});
			}
		}
	}
	int mn = INF;
	for(int i = 0; i <= n; i++){ mn = min<int>(mn, D[e][i]); }
	po(mn);
	int P = 0;
	while(k--){
		mn = INF;
