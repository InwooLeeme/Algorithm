...
struct SegTree{
	...
	SegTree():tree(sz << 1, 1),lazy(sz << 1){}
	
	void Push(int node){
		if(!lazy[node]) return;
		if(node < sz) for(const auto nxt : { node << 1, node << 1 | 1}) lazy[nxt] = lazy[node];
		tree[node] = lazy[node]; lazy[node] = 0;
	}
	
	void Update(int l, int r, int val, int node = 1, int s = 1, int e = sz){
		Push(node);
		if(r < s or l > e) return;
		if(l <= s and e <= r) { lazy[node] = val, Push(node); return;}
		int mid = s + e >> 1;
		Update(l, r, val, node << 1, s, mid);
		Update(l, r, val, node << 1 | 1, mid + 1, e);
...
void Main(){
	...
	SegTree<1 << 17> ST;
	while(q--){
		char c; int x,y,z; ri(c, x, y); if(x > y) swap(x, y);
		if(c == 'C'){
			ri(z); ST.Update(x, y, 1 << (z - 1));
		}
		else po(__builtin_popcount(ST.Query(x, y)));
	}
}
