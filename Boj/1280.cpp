...
void Main(){
	...
	int ret = 1;
	ST1.Update(v[1], 1); ST2.Update(v[1], v[1]);
	const int mx = 200'000;
	for(int i = 2; i <= n; i++){
		int x = v[i];
		int lcnt = ST1.Query(0, x - 1);
		int rcnt = ST1.Query(x + 1, mx);
		int lv = ST2.Query(0, x - 1);
		int rv = ST2.Query(x + 1, mx);
		int lret = x * lcnt - lv; lret %= MOD;
		int rret = rv - x * rcnt; rret %= MOD;
		int res = lret + rret; res %= MOD;
		ret *= res; ret %= MOD;
		ST1.Update(x, 1); ST2.Update(x, x);
	}
	po(ret % MOD);
}
